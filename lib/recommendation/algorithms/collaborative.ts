...
import { RecommendationCandidate, UserProfile, ContentFeatures, RecommendationRequest, RecommendationSource, UserAction, UserActionType, }
from '../types' 

import { ACTION_WEIGHTS }
from '../config' /** * 用户-物品交互矩阵 */
interface UserItemMatrix { [userId: string]: { [itemId: string]: number // 交互分数 }
}/** * 用户相似度 */
interface UserSimilarity { user1: string user2: string similarity: number }
/** * 协同过滤推荐算法 */
export class CollaborativeFilteringAlgorithm extends BaseRecommendationAlgorithm { name = 'collaborative-filtering' // 缓存 private userItemMatrix: UserItemMatrix | null = null private userSimilarities: Map<string, UserSimilarity[]> = new Map() private itemSimilarities: Map<string, Map<string, number>> = new Map() async generateCandidates( userProfile: UserProfile | null, request: RecommendationRequest, contentPool: ContentFeatures[] ): Promise<RecommendationCandidate[]> { // 过滤候选内容 const filteredContent = this.filterCandidates(contentPool, request, userProfile) // 如果没有用户信息，返回空结果（协同过滤需要用户历史） if (!userProfile || !request.user_id) { return []
}
// 获取用户行为数据（这里简化处理，实际需要从数据库获取） const userActions = await this.getUserActions(request.user_id) if (userActions.length < 5) { // 用户行为太少，无法进行协同过滤 return []
}
// 构建用户-物品矩阵 await this.buildUserItemMatrix() // 基于用户的协同过滤 const userBasedCandidates = await this.userBasedRecommendation( request.user_id, filteredContent, request.count || 10 ) // 基于物品的协同过滤 const itemBasedCandidates = await this.itemBasedRecommendation( request.user_id, userActions, filteredContent, request.count || 10 ) // 合并结果 const candidateMap = new Map<string, RecommendationCandidate>() // 合并用户协同过滤结果 for (const candidate of userBasedCandidates) { candidateMap.set(candidate.post_id, candidate) }
// 合并物品协同过滤结果 for (const candidate of itemBasedCandidates) { const existing = candidateMap.get(candidate.post_id) if (existing) { // 取较高分数 existing.score = Math.max(existing.score, candidate.score) existing.features = { ...existing.features, ...candidate.features, }
}
else { candidateMap.set(candidate.post_id, candidate) }
}
...
// 转换为候选列表 const candidates: RecommendationCandidate[] = []
const contentMap = new Map(contentPool.map(c => [c.post_id, c])) for (const [itemId, score]
of itemScores.entries()) { const content = contentMap.get(itemId) if (!content) continue const reasons = itemReasons.get(itemId) || []
const candidate: RecommendationCandidate = { post_id: itemId, score: this.score(content, null, { collaborative_score: score }), reasons: reasons.length > 0 ? [`与「${reasons[0]
}」相似`] : ['基于您的阅读历史'], source: RecommendationSource.COLLABORATIVE, features: { similarity_score: score, quality_score: content.quality_score, }, }
candidates.push(candidate) }
return this.sortCandidates(candidates, limit) }
/** * 查找相似用户 */
private async findSimilarUsers(userId: string, limit: number): Promise<UserSimilarity[]> { // 检查缓存 if (this.userSimilarities.has(userId)) { return this.userSimilarities.get(userId)!.slice(0, limit) }
if (!this.userItemMatrix || !this.userItemMatrix[userId]) { return []
}
const similarities: UserSimilarity[] = []
const targetUserItems = this.userItemMatrix[userId] // 计算与其他用户的相似度 for (const [otherUserId, otherUserItems]
of Object.entries(this.userItemMatrix)) { if (otherUserId === userId) continue const similarity = this.calculateUserSimilarity(targetUserItems, otherUserItems) if (similarity > 0.1) { // 最小相似度阈值 similarities.push({ user1: userId, user2: otherUserId, similarity, }) }
}
// 排序并缓存 similarities.sort((a, b) => b.similarity - a.similarity) this.userSimilarities.set(userId, similarities) return similarities.slice(0, limit) }
/** * 查找相似物品 */
private async findSimilarItems(itemId: string, limit: number): Promise<Map<string, number>> { // 检查缓存 if (this.itemSimilarities.has(itemId)) { return this.itemSimilarities.get(itemId)! }
if (!this.userItemMatrix) { return new Map() }
// 获取与该物品交互过的用户 const itemUsers: Map<string, number> = new Map() for (const [userId, items]
of Object.entries(this.userItemMatrix)) { if (items[itemId]) { itemUsers.set(userId, items[itemId]) }
}
if (itemUsers.size === 0) { return new Map() }
// 计算与其他物品的相似度 const similarities: Map<string, number> = new Map() const allItems = new Set<string>() // 收集所有物品 for (const items of Object.values(this.userItemMatrix)) { for (const item of Object.keys(items)) { allItems.add(item) }
}
...
// TODO: 从数据库获取所有用户行为并构建矩阵 // 示例代码： // const allActions = await getAllUserActions() // for (const action of allActions) { // if (!this.userItemMatrix[action.user_id]) { // this.userItemMatrix[action.user_id] = {}
// }
// const weight = ACTION_WEIGHTS[action.action_type] || 1 // const current = this.userItemMatrix[action.user_id][action.target_id] || 0 // this.userItemMatrix[action.user_id][action.target_id] = current + weight // }
}/** * 获取用户行为（模拟） */
private async getUserActions(userId: string): Promise<UserAction[]> { // TODO: 实际需要从数据库获取 return []
} }