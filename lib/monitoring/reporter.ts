...
import { getMetricDisplayName, getMetricUnit } from './config'
import { Statistics } from '@/lib/analytics/algorithms'
...
<head>
  ...
  <style>
    ...
    .score { font-size: 48px; font-weight: bold; color: ${report.summary.overall_score >= 80 ? '#28a745' : report.summary.overall_score >= 50 ? '#ffc107' : '#dc3545'}; }
    .insights { background: #e9ecef; padding: 15px; border-radius: 8px; margin: 10px 0; }
...
<body>
  <h1>性能报告</h1>
  <p><strong>报告周期：</strong>${new Date(report.period.start).toLocaleString()} - ${new Date(report.period.end).toLocaleString()}</p>
  
  <div class="summary">
    <h2>总体评分</h2>
    <div class="score">${report.summary.overall_score}</div>
    <p><strong>评级：</strong>${ratingEmoji[report.summary.rating]} ${report.summary.rating}</p>
    
    <h3>关键洞察</h3>
    <ul class="insights">
      ${report.summary.key_insights.map(insight => `<li>${insight}</li>`).join('')}
    </ul>
  </div>
  ...
  <table>
    ...
    ${Object.entries(report.web_vitals).map(([key, value]) => `
      <tr>
        <td>${getMetricDisplayName(key.toUpperCase() as MetricType)}</td>
        <td>${value.p75.toFixed(2)} ${getMetricUnit(key.toUpperCase() as MetricType)}</td>
        <td>${value.p95.toFixed(2)} ${getMetricUnit(key.toUpperCase() as MetricType)}</td>
        <td>
          🟢 ${value.rating_distribution.good} 
          🟡 ${value.rating_distribution.needs_improvement} 
          🔴 ${value.rating_distribution.poor}
        </td>
      </tr>
    `).join('')}
  </table>
  ...
  <div class="metric">
    <span>平均延迟</span>
    <span>${report.api_performance.average_latency.toFixed(2)}ms</span>
  </div>
  <div class="metric">
    <span>P95 延迟</span>
    <span>${report.api_performance.p95_latency.toFixed(2)}ms</span>
  </div>
  <div class="metric">
    <span>错误率</span>
    <span>${(report.api_performance.error_rate * 100).toFixed(2)}%</span>
  </div>
  <div class="metric">
    <span>总请求数</span>
    <span>${report.api_performance.total_requests}</span>
  </div>
  ...
  ${report.recommendations.map(rec => `
    <div class="recommendation ${rec.priority}">
      ...
      <p><strong>潜在影响：</strong>${rec.potential_impact}</p>
    </div>
  `).join('')}
