...
export class MonitoringCollector {
   ...
   * 记录 Web Vital 指标
   */
  private recordWebVital(metricType: MetricType, metric: Metric) {
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        if (entry.entryType === 'navigation') {
          ...
          this.recordMetric(MetricType.PAGE_LOAD_TIME, loadTime, {
   ...
   * 记录 API 性能
   */
  public recordAPIMetric(metric: APIMetric) {
    const dataPoint: MetricDataPoint = {
      metric_type: MetricType.API_LATENCY,
      value: metric.duration,
      unit: 'ms',
      rating: getPerformanceRating(MetricType.API_LATENCY, metric.duration),
    ...
    if (metric.status_code >= 400) {
      this.recordMetric(MetricType.API_ERROR_RATE, 1, {
