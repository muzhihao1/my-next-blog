...
interface BatchedAction extends TrackActionOptions {
  timestamp: string
}

class ActionTracker {
  private batchQueue: BatchedAction[] = []
  private batchTimer: NodeJS.Timeout | null = null
  private readonly batchSize = 10
  private readonly batchDelay = 5000 // 5秒

  /**
   * 追踪用户行为
   * 使用批量发送来优化性能
   */
  async trackAction(options: TrackActionOptions): Promise<void> {
   ...
   * 追踪页面浏览
   */
  async trackPageView(options: Omit<TrackActionOptions, 'actionType'>): Promise<void> {
   ...
   * 追踪滚动深度
   */
  trackScrollDepth(contentId: string, contentType: ContentType, depth: number): void {
    // 只在特定深度记录（25%, 50%, 75%, 100%）
    const thresholds = [25, 50, 75, 100]
    if (!thresholds.includes(depth)) return

    // 避免重复记录
    const key = `scroll_${contentId}_${depth}`
   ...
   * 计算阅读时长
   */
  startReadingTimer(contentId: string): () => number {
  ...
  async flush(): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to track actions')
      }
    } catch (error) {
   ...
   * 重试发送失败的行为数据
   */
  async retryPendingActions(): Promise<void> {
    const stored = localStorage.getItem('pending_actions')
    if (!stored) return
