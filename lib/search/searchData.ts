...
export function buildSearchIndex(): SearchItem[] {
  ...
  fallbackBooks.forEach(book => {
    searchItems.push({
      id: book.id,
      title: book.title,
      content: book.notes || book.takeaways || '',
      type: 'book',
      url: `/bookshelf/${book.id}`,
      author: book.author,
...
export async function getSearchData(): Promise<SearchItem[]> {
  // 在实际应用中，这里可以从 Notion API 获取最新数据
  // 现在先使用 fallback 数据
...
export function highlightText(text: string, query: string): string {
  ...
  return text.replace(regex, '<mark class="bg-yellow-200 dark:bg-yellow-800 text-black dark:text-white rounded px-0.5">$1</mark>')
}