...
describe('useDebounce', () => {
  ...
  it('cancels previous timeout on rapid changes', () => {
    ...
    // Total time passed: 600ms, but each change reset the timer
    // So the value should still be 'first'
  ...
  it('handles immediate updates with zero delay', () => {
    const { result, rerender } = renderHook(
  ...
  it('maintains referential equality for objects when value does not change', () => {
    const obj = { count: 1 }
    const { result, rerender } = renderHook(() => useDebounce(obj, 500))
    
    const firstResult = result.current
    
    // Re-render without changing the value
    rerender()
