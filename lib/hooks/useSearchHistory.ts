 ...
 * - Limits history to MAX_HISTORY_ITEMS
...
export function useSearchHistory() {
   ...
   */
  const addToHistory = useCallback((query: string, resultCount: number, type?: SearchHistoryItem['type']) => {
  ...
  /**
   * Get popular queries (queries with most results)
   */
  const getPopularQueries = useCallback((limit: number = 5) => {
    const queryMap = new Map<string, { query: string, totalResults: number, count: number }>()
    
    history.forEach(item => {
      const key = item.query.toLowerCase()
      const existing = queryMap.get(key) || { query: item.query, totalResults: 0, count: 0 }
      queryMap.set(key, {
