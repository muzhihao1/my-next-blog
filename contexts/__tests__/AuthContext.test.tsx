...
describe('AuthContext', () => {
  ...
  it('handles sign out error', async () => {
    (fetch as jest.Mock)
      ...
      .mockResolvedValueOnce({
        ok: false,
        json: async () => ({ error: 'Sign out failed' }),
      })
    ...
    expect(result.current.user).toBeNull() // Still clears user on error
    
    consoleSpy.mockRestore()
  })

  it('stores auth token in localStorage on sign in', async () => {
    ...
    expect(window.localStorage.getItem('auth-token')).toBe(mockToken)
  })

  it('clears auth token from localStorage on sign out', async () => {
