...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = async () => {
    try {
      ...
      if (error) {
        const errorMessage = error.message === 'Email link is invalid or has expired' 
          ? '登录链接已失效，请重新登录'
  ...
  return (
    <AuthContext.Provider value={{ user, loading, error, signIn, signOut, refreshUser, clearError }}>
      {children}
