...
interface UseInteractionsReturn {
  ...
  refetchInteractions: () => Promise<void>
}
...
export function useInteractions(
  ...
  const { initialLikes = 0, initialBookmarks = 0 } = options
  ...
  const fetchInteractions = useCallback(async () => {
    try {
      ...
      const likesResponse = await fetch(`/api/likes?${likesParams}`)
      ...
      const bookmarksResponse = await fetch(`/api/bookmarks?${bookmarksParams}`)
      const bookmarksData = await bookmarksResponse.json()

      setLikes(likesData.count || 0)
      setIsLiked(likesData.isLiked || false)
      setBookmarks(bookmarksData.count || 0)
      setIsBookmarked(bookmarksData.isBookmarked || false)
    } catch (error) {
  ...
  const toggleLike = async () => {
    ...
    try {
      ...
      // 如果是DELETE请求，需要传递查询参数
      if (isLiked) {
        const params = new URLSearchParams({ contentId, contentType })
        const deleteResponse = await fetch(`/api/likes?${params}`, {
          method: 'DELETE'
        })
  ...
  const toggleBookmark = async () => {
    ...
    try {
      ...
      if (isBookmarked) {
        const params = new URLSearchParams({ contentId, contentType })
        const deleteResponse = await fetch(`/api/bookmarks?${params}`, {
          method: 'DELETE'
        })
