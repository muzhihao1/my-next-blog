...
const triggerIntersection = (isIntersecting: boolean) => {
  const entries = observedElements.map(target => ({
    isIntersecting,
    target,
...
describe('LazyLoad', () => {
  ...
  it('renders placeholder initially', () => {
    ...
    expect(placeholder).toHaveClass('bg-gray-100', 'dark:bg-gray-800', 'rounded-lg')
  ...
  it('calls onVisible callback when becoming visible', async () => {
  ...
  it('uses custom threshold and rootMargin', () => {
    const observerSpy = jest.fn().mockImplementation((callback, options) => {
  ...
  it('does not trigger multiple times', async () => {
    const onVisibleMock = jest.fn()
    ...
    await waitFor(() => {
      // Should only be called once due to disconnect
      expect(onVisibleMock).toHaveBeenCalledTimes(1)
    })
  })

  it('does not render children when not intersecting', async () => {
    render(
      <LazyLoad>
  ...
  it('handles null container ref gracefully', () => {
    // Temporarily override ref to be null
    const originalRef = Object.getOwnPropertyDescriptor(HTMLDivElement.prototype, 'offsetParent')
    Object.defineProperty(HTMLDivElement.prototype, 'offsetParent', {
      get: () => null,
      configurable: true
    })
    ...
    // Should still call observe (React handles ref assignment)
    expect(observeSpy).toHaveBeenCalled()

    // Restore
    if (originalRef) {
      Object.defineProperty(HTMLDivElement.prototype, 'offsetParent', originalRef)
    }
  })
})