...
import Container, { PageContainer, ContentContainer } from '../Container'
...
describe('Container', () => {
  ...
  it('applies custom size classes', () => {
    ...
    Object.entries(sizes).forEach(([size, expectedClass]) => {
      const { container } = render(
        <Container size={size as any}>Content</Container>
      )
      
      const containerDiv = container.firstChild
      expect(containerDiv).toHaveClass(expectedClass)
    })
  })
  ...
  it('removes padding when noPadding is true', () => {
    ...
    expect(containerDiv).not.toHaveClass('sm:px-6')
    expect(containerDiv).not.toHaveClass('lg:px-8')
  })
...
describe('PageContainer', () => {
  ...
  it('applies vertical padding', () => {
    ...
    const containerDiv = container.firstChild
    expect(containerDiv).toHaveClass('py-16')
  })
  ...
  it('combines custom className with default classes', () => {
    const { container } = render(
      <PageContainer className="custom-page-class">Content</PageContainer>
    )
    
    const containerDiv = container.firstChild
    expect(containerDiv).toHaveClass('py-16', 'custom-page-class')
  })

  it('always includes padding (no noPadding prop)', () => {
    const { container } = render(
      <PageContainer>Content</PageContainer>
    )
...
describe('ContentContainer', () => {
  ...
  it('uses sm size for readability', () => {
    const { container } = render(
  ...
  it('accepts custom className', () => {
    const { container } = render(
      <ContentContainer className="prose prose-lg">Content</ContentContainer>
    )
    
    const containerDiv = container.firstChild
    expect(containerDiv).toHaveClass('py-8', 'prose', 'prose-lg')
  })
