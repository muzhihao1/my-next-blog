...
const languageLoaders = {
  ...
  jsx: () => import('react-syntax-highlighter/dist/esm/languages/hljs/javascript'),
  tsx: () => import('react-syntax-highlighter/dist/esm/languages/hljs/typescript'),
...
export function OptimizedCodeBlock({
  ...
  useEffect(() => {
    if (normalizedLanguage === 'plaintext' || !languageLoaders[normalizedLanguage as keyof typeof languageLoaders]) {
      setIsLanguageLoaded(true)
      return
    }

    registerLanguage(normalizedLanguage).then(() => {
      setIsLanguageLoaded(true)
    })
  }, [normalizedLanguage])
  ...
  // 如果语言未加载，显示纯文本
  if (!isLanguageLoaded && normalizedLanguage !== 'plaintext') {
    return (
  ...
  return (
    <div className={`relative group ${className}`}>
      ...
      {/* 语言标签 */}
      {language !== 'plaintext' && (
        <div className="absolute top-2 left-2 px-2 py-1 text-xs bg-gray-700 text-gray-300 rounded opacity-75">
          {language}
        </div>
      )}
...
export async function preloadCommonLanguages() {
  const commonLanguages = ['javascript', 'typescript', 'python', 'bash', 'css']
  await Promise.all(commonLanguages.map(lang => registerLanguage(lang)))
}

/**
 * 简化的代码块组件（无语法高亮）
 * 用于不需要语法高亮的场景，进一步减小bundle
 */
export function SimpleCodeBlock({ 
  ...
  return (
    <div className={`relative group ${className}`}>
      <button
        onClick={handleCopy}
        className="absolute top-2 right-2 px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 text-gray-300 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10"
        aria-label="复制代码"
      >
        {copied ? '已复制' : '复制'}
      </button>
