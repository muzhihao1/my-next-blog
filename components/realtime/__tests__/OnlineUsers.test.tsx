...
describe('OnlineUsers', () => {
  ...
  it('renders online count in simple mode', async () => {
    const { getRealtimeClient } = require('@/lib/realtime/client')
    const mockClient = {
      subscribe: jest.fn().mockImplementation((_channel, callbacks) => {
        // Simulate connection
    ...
    expect(screen.getByText('2 人在线').previousElementSibling).toHaveClass('animate-ping')
  })

  it('renders user avatars in detailed mode', async () => {
    const { getRealtimeClient } = require('@/lib/realtime/client')
    const onlineUsers = [
      {
        userId: 'user-1',
    ...
    const mockClient = {
      ...
      unsubscribe: jest.fn().mockResolvedValue(undefined),
      getPresence: jest.fn().mockResolvedValue(onlineUsers),
      updatePresence: jest.fn().mockResolvedValue(undefined)
    }
    getRealtimeClient.mockReturnValue(mockClient)

    render(<OnlineUsers showDetails={true} />)
    ...
    await waitFor(() => {
      ...
      expect(avatars[1]).toHaveAttribute('src', expect.stringContaining('ui-avatars.com'))
      expect(avatars[1]).toHaveAttribute('alt', 'Bob')
    })
  })

  it('shows remaining count when exceeding maxDisplay', async () => {
    const { getRealtimeClient } = require('@/lib/realtime/client')
    const onlineUsers = Array.from({ length: 8 }, (_, i) => ({
      userId: `user-${i}`,
      username: `User ${i}`,
    ...
    render(<OnlineUsers showDetails={true} maxDisplay={5} />)

    await waitFor(() => {
      expect(screen.getByText('8 人在线')).toBeInTheDocument()
  ...
  it('shows tooltip on hover in detailed mode', async () => {
    ...
    // Hover over avatar container
    const avatarContainer = screen.getByRole('img', { name: 'Alice' }).parentElement?.parentElement
    await user.hover(avatarContainer!)

    await waitFor(() => {
      expect(screen.getByText('在线用户')).toBeInTheDocument()
      expect(screen.getByText('Alice')).toBeInTheDocument()
      expect(screen.getByText('正在浏览: /posts/hello-world')).toBeInTheDocument()
    })
  })

  it('updates user presence on mount', async () => {
    const { getRealtimeClient } = require('@/lib/realtime/client')
    const mockUpdatePresence = jest.fn().mockResolvedValue(undefined)
    const mockClient = {
      subscribe: jest.fn().mockImplementation((_channel, callbacks) => {
        setTimeout(() => callbacks.onConnect(), 100)
        return Promise.resolve()
  ...
  it('cleans up on unmount', async () => {
    ...
    await waitFor(() => {
      expect(mockUnsubscribe).toHaveBeenCalledWith('presence')
    })
  })

  it('handles custom channel name', async () => {
    const { getRealtimeClient } = require('@/lib/realtime/client')
    const mockClient = {
      subscribe: jest.fn().mockResolvedValue(undefined),
      unsubscribe: jest.fn().mockResolvedValue(undefined),
    ...
    render(<OnlineUsers channelName="custom-channel" />)
