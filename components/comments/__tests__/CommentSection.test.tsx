...
describe('CommentSection', () => {
  ...
  it('loads and displays comments', async () => {
    ...
    // Verify API call
    expect(fetch).toHaveBeenCalledWith('/api/posts/post-1/comments')
  })
  ...
  it('shows empty state when no comments', async () => {
    ...
    expect(screen.getByText('成为第一个评论的人吧！')).toBeInTheDocument()
  })
  ...
  it('handles comment deletion', async () => {
    ...
    expect(deleteButtons).toHaveLength(1) // Only one comment is by the current user
  ...
  it('loads more comments when scrolling', async () => {
    const manyComments = Array.from({ length: 15 }, (_, i) => ({
      ...
      updatedAt: new Date(`2024-01-01T${10 + i}:00:00`),
      user: mockUser,
    }))
    ...
    (fetch as jest.Mock)
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ 
          comments: manyComments.slice(0, 10),
          hasMore: true,
        }),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ 
          comments: manyComments.slice(10, 15),
          hasMore: false,
        }),
      })
    ...
    expect(fetch).toHaveBeenCalledWith('/api/posts/post-1/comments?offset=10')
    
    // Additional comments should appear
    await waitFor(() => {
      expect(screen.getByText('Comment 10')).toBeInTheDocument()
    })
    
    // Load more button should be hidden when no more comments
  ...
  it('toggles comment form visibility', async () => {
    ...
    // Click to hide
    const toggleButton = screen.getByRole('button', { name: '隐藏评论表单' })
    ...
    // Click to show again
    const showButton = screen.getByRole('button', { name: '显示评论表单' })
