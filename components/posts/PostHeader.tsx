...
import { Twitter, Linkedin, Link2, Heart, Bookmark, Share2 } from 'lucide-react'
...
export function PostHeader({ post, readingTime = 6, wordCount = 593 }: PostHeaderProps) {
  const [isLiked, setIsLiked] = useState(false)
  const [isBookmarked, setIsBookmarked] = useState(false)

  const publishDate = new Date(post.date)
  const lastModified = post.lastModified ? new Date(post.lastModified) : publishDate
    ...
    <header className="mb-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <Link 
                href={`/authors/${post.author.id || post.author.name}`}
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
              ...
              {lastModified > publishDate && (
                <>
                  <span className="text-gray-400 dark:text-gray-600">·</span>
                  <span>
                    更新于 {formatDistanceToNow(lastModified, { locale: zhCN, addSuffix: true })}
                  </span>
                </>
              )}
            </div>
      ...
      <div className="flex items-center justify-between py-4 border-t border-gray-200 dark:border-gray-800">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-gray-500 dark:text-gray-400 mr-2">分享：</span>
