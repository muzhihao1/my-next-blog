...
describe('ShareButton', () => {
  ...
  it('opens share menu on click', async () => {
    ...
    // Check share options
    expect(screen.getByRole('menuitem', { name: '分享到 Twitter' })).toBeInTheDocument()
    expect(screen.getByRole('menuitem', { name: '分享到 Facebook' })).toBeInTheDocument()
    expect(screen.getByRole('menuitem', { name: '分享到 LinkedIn' })).toBeInTheDocument()
    expect(screen.getByRole('menuitem', { name: '复制链接' })).toBeInTheDocument()
  })

  it('uses native share API when available', async () => {
    // Mock canShare to return true
    ...
    const nativeShareButton = screen.getByRole('menuitem', { name: '系统分享' })
    expect(nativeShareButton).toBeInTheDocument()
    
    // Click native share
    await user.click(nativeShareButton)
    
    // Verify navigator.share was called
    expect(navigator.share).toHaveBeenCalledWith({
      title: 'Test Post Title',
      text: 'This is a test post description',
      url: 'https://example.com/post/test-post',
    })
  })
  ...
  it('handles native share error gracefully', async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith('Native share failed:', expect.any(Error))
    })
  ...
  it('shares to Twitter', async () => {
    ...
    const twitterButton = screen.getByRole('menuitem', { name: '分享到 Twitter' })
    await user.click(twitterButton)
    
    // Verify window.open was called with Twitter share URL
    expect(global.open).toHaveBeenCalledWith(
      expect.stringContaining('https://twitter.com/intent/tweet'),
      '_blank'
    )
    
    const url = (global.open as jest.Mock).mock.calls[0][0]
    expect(url).toContain('text=Test%20Post%20Title')
  ...
  it('shares to Facebook', async () => {
    ...
    const facebookButton = screen.getByRole('menuitem', { name: '分享到 Facebook' })
    await user.click(facebookButton)
    
    // Verify window.open was called with Facebook share URL
    expect(global.open).toHaveBeenCalledWith(
  ...
  it('shares to LinkedIn', async () => {
    ...
    const linkedinButton = screen.getByRole('menuitem', { name: '分享到 LinkedIn' })
    await user.click(linkedinButton)
    
    // Verify window.open was called with LinkedIn share URL
    expect(global.open).toHaveBeenCalledWith(
  ...
  it('copies link to clipboard', async () => {
    ...
    const copyButton = screen.getByRole('menuitem', { name: '复制链接' })
  ...
  it('applies custom className', () => {
    render(<ShareButton {...defaultProps} className="custom-share-button" />)
    
    const button = screen.getByRole('button', { name: '分享' })
    expect(button).toHaveClass('custom-share-button')
  })
  ...
  it('handles keyboard navigation', async () => {
    ...
    const firstMenuItem = screen.getByRole('menuitem', { name: '分享到 Twitter' })
  ...
  it('shows share count when provided', () => {
    render(<ShareButton {...defaultProps} shareCount={42} />)
    
    expect(screen.getByText('42')).toBeInTheDocument()
  })

  it('formats large share counts', () => {
    render(<ShareButton {...defaultProps} shareCount={1234} />)
