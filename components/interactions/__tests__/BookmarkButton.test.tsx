...
describe('BookmarkButton', () => {
  ...
  it('renders unbookmarked state by default', () => {
    ...
    // Check for unfilled bookmark icon
  ...
  it('toggles bookmark when clicked by authenticated user', async () => {
    ...
    // Verify API call
    expect(fetch).toHaveBeenCalledWith('/api/posts/post-1/bookmark', {
      method: 'POST',
    })
  ...
  it('removes bookmark when clicked on bookmarked post', async () => {
    (fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
    ...
    // Should show success message
    expect(screen.getByText('已从收藏夹移除')).toBeInTheDocument()
  })
  ...
  it('shows login prompt when clicked by unauthenticated user', async () => {
    ...
    expect(screen.getByText('请先登录后再收藏')).toBeInTheDocument()
  })
  ...
  it('applies custom className', () => {
    ...
    const button = screen.getByRole('button', { name: '收藏' })
    expect(button).toHaveClass('custom-bookmark-button')
  })
  ...
  it('hides toast message after timeout', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('已添加到收藏夹')).not.toBeInTheDocument()
    })
  ...
  it('handles rapid clicks correctly', async () => {
    ...
    // Should only make one API call (second click ignored due to loading state)
  ...
  it('shows optimistic UI update', async () => {
    ...
    expect(button).toHaveAttribute('aria-pressed', 'true')
    expect(button.querySelector('svg')).toHaveClass('fill-yellow-500')
