...
import { Calendar, Eye, Clock, TrendingUp, Users, MessageSquare } from 'lucide-react'
...
export function PostAnalytics({ postId, className = '' }: PostAnalyticsProps) {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        if (!response.ok) throw new Error('获取文章分析数据失败')
  ...
  // 准备图表数据
  const viewsChartData = data.views_over_time.map(item => ({
    x: new Date(item.date).toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' }),
    y: item.views,
    label: `${item.views} 次浏览`
  }))

  const referrerData = Object.entries(data.referrer_breakdown)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([source, count]) => ({
      label: source === 'direct' ? '直接访问' : 
             source === 'search' ? '搜索引擎' :
             source === 'social' ? '社交媒体' : source,
      value: count
    }))

  const geoData = Object.entries(data.geographic_breakdown)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 8)
    .map(([country, count]) => ({
      label: country,
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400">文章数据分析</p>
      </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold">
                {Math.floor(data.avg_read_time / 60)}:{(data.avg_read_time % 60).toString().padStart(2, '0')}
              </p>
              <p className="text-sm text-gray-500">平均阅读时长</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">完读率</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">参与度评分</p>
          </div>
      ...
      {data.related_posts && data.related_posts.length > 0 && (
        <Card className="p-6">
          ...
          <div className="space-y-3">
            {data.related_posts.map((post, index) => (
              <div key={post.post_id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-800 last:border-0">
                <div>
                  <p className="font-medium">{post.title}</p>
                  <p className="text-sm text-gray-500">相关度 {(post.correlation_score * 100).toFixed(0)}%</p>
                </div>
