...
export function EngagementFunnel({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const progress = animate ? easeOutCubic(progressRef.current) : 1

      // Set styles
      const textColor = theme === 'dark' ? '#F3F4F6' : '#111827'
      const bgColor = theme === 'dark' ? '#111827' : '#FFFFFF'
      const labelBgColor = theme === 'dark' ? '#1F2937' : '#F3F4F6'
      ...
      const padding = { top: title ? 60 : 40, right: 40, bottom: 40, left: 40 }
      const funnelWidth = rect.width - padding.left - padding.right
      const funnelHeight = rect.height - padding.top - padding.bottom
      ...
      stages.forEach((stage, index) => {
        ...
        const topWidth = index === 0 
          ? funnelWidth 
          : (stages[index - 1].value / maxValue) * funnelWidth * 0.9
        const bottomWidth = (stage.value / maxValue) * funnelWidth * 0.9
        const currentTopWidth = topWidth * stageProgress
        const currentBottomWidth = bottomWidth * stageProgress
        ...
        ctx.fillStyle = color
        ctx.beginPath()
        ctx.moveTo(centerX - currentTopWidth / 2, y)
        ctx.lineTo(centerX + currentTopWidth / 2, y)
        ctx.lineTo(centerX + currentBottomWidth / 2, y + stageHeight - 5)
        ctx.lineTo(centerX - currentBottomWidth / 2, y + stageHeight - 5)
        ctx.closePath()
        ctx.fill()

        // Add gradient overlay
        const gradient = ctx.createLinearGradient(0, y, 0, y + stageHeight)
        ...
        if (stageProgress === 1) {
          ...
          const valueText = stage.value.toLocaleString()
          const percentageText = `${((stage.value / maxValue) * 100).toFixed(1)}%`
          ...
          if (showPercentage) {
            ...
            ctx.fillText(
              percentageText,
              centerX + currentTopWidth / 2 + 20,
              y + stageHeight / 2
            )
          }

          // Draw dropoff rate
          if (showDropoff && index > 0) {
      ...
      // Draw conversion rate summary
      if (stages.length > 1 && progressRef.current === 1) {
  ...
  }, [stages, title, showPercentage, showDropoff, animate, theme])
