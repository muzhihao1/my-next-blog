...
export function BarChart({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const padding = orientation === 'vertical' 
        ...
        : { top: 40, right: 80, bottom: 60, left: 120 }
      ...
      const gap = 0.2 // 20% gap between bars
      ...
      data.forEach((item, index) => {
        ...
        if (orientation === 'vertical') {
          const barWidth = chartWidth / barCount * (1 - gap)
          const barX = padding.left + (index + gap / 2) * (chartWidth / barCount)
          ...
          if (showValues && progress === 1) {
            ...
            ctx.fillText(formatValue(item.value), barX + barWidth / 2, barY - 5)
          }

          // Draw x-axis label
          ctx.save()
          ctx.translate(barX + barWidth / 2, padding.top + chartHeight + 10)
          ...
          // Horizontal bars
          const barHeight = chartHeight / barCount * (1 - gap)
          const barY = padding.top + (index + gap / 2) * (chartHeight / barCount)
          ...
          if (showValues && progress === 1) {
            ...
            ctx.textAlign = 'left'
            ctx.textBaseline = 'middle'
            ctx.fillText(formatValue(item.value), barX + barWidth + 5, barY + barHeight / 2)
          }
          ...
          ctx.textAlign = 'right'
          ctx.textBaseline = 'middle'
          ctx.fillText(item.label, padding.left - 10, barY + barHeight / 2)
        }
      })
      ...
      if (data.length > 1) {
        ...
        const legendStartX = (rect.width - data.length * legendItemWidth) / 2
  ...
  }, [data, title, xLabel, yLabel, orientation, showValues, animate, colors, formatValue, theme])
