...
export function LineChart({
  ...
  useEffect(() => {
    ...
    const yScale = (y: number) => padding.top + chartHeight - ((y - minY + yPadding) / (yRange + 2 * yPadding)) * chartHeight
    ...
    for (let i = 0; i <= 5; i++) {
      const value = minY - yPadding + (i / 5) * (yRange + 2 * yPadding)
      const y = padding.top + chartHeight - (i / 5) * chartHeight
      ctx.fillText(formatY(value), padding.left - 10, y)
    }
    ...
    for (let i = 0; i < data.length; i += labelInterval) {
      const x = xScale(i)
      ctx.fillText(formatX(data[i].x), x, padding.top + chartHeight + 10)
    }
    ...
    if (data.length > 0) {
      ...
      data.forEach((point, i) => {
        ctx.lineTo(xScale(i), yScale(point.y))
      })
      ...
      data.forEach((point, i) => {
        if (i === 0) {
          ctx.moveTo(xScale(i), yScale(point.y))
        } else {
          ctx.lineTo(xScale(i), yScale(point.y))
        }
      })
      ...
      if (showDots) {
        ...
        data.forEach((point, i) => {
          ctx.beginPath()
          ctx.arc(xScale(i), yScale(point.y), 4, 0, Math.PI * 2)
          ctx.fill()
          
          // Draw value label on hover (simplified)
          if (point.label) {
            ctx.save()
            ctx.fillStyle = bgColor
            ctx.strokeStyle = color
            ctx.lineWidth = 1
            const labelWidth = ctx.measureText(point.label).width + 10
            const labelX = xScale(i)
            const labelY = yScale(point.y) - 15
  ...
  }, [data, title, xLabel, yLabel, showGrid, showDots, color, formatX, formatY, theme])
