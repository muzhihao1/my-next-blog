...
export function PieChart({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      data.forEach((item, index) => {
        ...
        ctx.arc(centerX, centerY, isHovered ? radius * 1.05 : radius, currentAngle, currentAngle + sliceAngle)
        if (donut) {
        ...
        // Draw labels
        if (showLabels && progress === 1 && item.value > 0) {
      ...
      if (showLabels && progress === 1) {
        const legendStartY = title ? size - (data.length * 20 + 20) : size - (data.length * 20 + 10)
        const legendX = 20
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      if (distance <= radius && distance >= innerRadius) {
        ...
        if (foundIndex !== hoverIndexRef.current) {
  ...
  }, [data, title, size, donut, showLabels, showPercentage, animate, colors, formatValue, theme])
