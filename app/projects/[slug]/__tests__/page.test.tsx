...
const mockProjects = [
  {
    ...
    content: '# Project Overview\n\nThis is the project content in **markdown**.',
    ...
    keyFeatures: ['Real-time updates', 'Mobile responsive', 'Dark mode support'],
    developmentProcess: 'We used agile methodology with 2-week sprints',
    codeSnippets: [
      { filename: 'app.tsx', language: 'typescript', code: 'const App = () => <div>Hello</div>' }
    ],
    challenges: ['Performance optimization', 'Cross-browser compatibility'],
    solutions: ['Implemented lazy loading', 'Used polyfills for older browsers'],
    metrics: {
      users: 5000,
  ...
  remark: () => ({
    use: () => ({
      process: async (content: string) => ({
        toString: () => `<h1>Project Overview</h1><p>This is the project content in <strong>markdown</strong>.</p>`
      })
    })
  })
}))
...
jest.mock('@/components/features/CodeSnippet', () => ({
  CodeSnippet: ({ files }: any) => (
    <div data-testid="code-snippet">
      {files.map((file: any, idx: number) => (
        <div key={idx} data-filename={file.filename}>
          {file.code}
        </div>
      ))}
    </div>
...
jest.mock('@/components/features/SocialShare', () => ({
  SocialShare: ({ url, title, description, tags }: any) => (
...
jest.mock('@/components/features/FavoriteButton', () => ({
  __esModule: true,
  default: ({ itemId, itemType, size }: any) => (
    <button data-testid="favorite-button" data-item-id={itemId} data-size={size}>
      Favorite
    </button>
  )
}))
...
describe('Project Detail Page', () => {
  ...
  describe('generateStaticParams', () => {
    it('returns slugs from getProjects when available', async () => {
    ...
    it('returns fallback slugs when getProjects returns empty', async () => {
  ...
  describe('generateMetadata', () => {
    it('generates metadata for existing project from API', async () => {
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(mockProjects[0])
      ...
      expect(metadata.openGraph).toMatchObject({
        ...
        description: 'A comprehensive test project',
        type: 'website',
        images: ['/images/project-thumb.jpg']
      })
    })

    it('generates metadata from fallback when API returns null', async () => {
    ...
    it('generates 404 metadata for non-existent project', async () => {
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(null)
  ...
  describe('Project Component', () => {
    it('renders complete project with all features', async () => {
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(mockProjects[0])
      ...
      // External links
      const demoLink = screen.getByRole('link', { name: /在线演示/ })
      ...
      // Metrics
      expect(screen.getByText('5,000')).toBeInTheDocument() // users
      expect(screen.getByText('95/100 Lighthouse score')).toBeInTheDocument()
      expect(screen.getByText('Featured on Product Hunt')).toBeInTheDocument()

      // Screenshots
      expect(screen.getByAltText('Test Project 截图 1')).toHaveAttribute('src', '/screenshot1.jpg')
      expect(screen.getByAltText('Test Project 截图 2')).toHaveAttribute('src', '/screenshot2.jpg')
      ...
      // Development process
      expect(screen.getByText('We used agile methodology with 2-week sprints')).toBeInTheDocument()
    ...
    it('renders minimal project without optional fields', async () => {
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(mockProjects[1])

      render(await ProjectDetailPage({ params: Promise.resolve({ slug: 'minimal-project' }) }))
    ...
    it('falls back to fallbackProjects when API returns null', async () => {
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(null)
    ...
    it('renders social share with correct props', async () => {
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(mockProjects[0])
    ...
    it('renders favorite button with correct props', async () => {
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(mockProjects[0])
    ...
    it('applies correct status badge styles', async () => {
      const activeProject = { ...mockProjects[1], status: 'active' as const }
      ...
      // Test active status
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(activeProject)
      const { rerender } = render(await ProjectDetailPage({ params: Promise.resolve({ slug: 'test' }) }))
      expect(screen.getByText('进行中')).toHaveClass('bg-green-100', 'text-green-700')

      // Test completed status
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(completedProject)
      rerender(await ProjectDetailPage({ params: Promise.resolve({ slug: 'test' }) }))
      expect(screen.getByText('已完成')).toHaveClass('bg-blue-100', 'text-blue-700')

      // Test archived status
      ;(getProjectBySlug as jest.Mock).mockResolvedValue(archivedProject)
      rerender(await ProjectDetailPage({ params: Promise.resolve({ slug: 'test' }) }))
    ...
    it('formats dates correctly', async () => {
      ...
      expect(screen.getByText('2024/1/1')).toBeInTheDocument() // Start date
      expect(screen.getByText('2024/6/1')).toBeInTheDocument() // End date
    })
