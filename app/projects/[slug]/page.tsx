    ...
    <div className="py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            {project.githubUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2 bg-gray-800 dark:bg-gray-700 text-white rounded-lg font-medium hover:bg-gray-900 dark:hover:bg-gray-600 transition-colors"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-6">
            <h3 className="font-semibold text-gray-900 dark:text-white mb-4">时间线</h3>
          ...
          {project.metrics && (
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-6">
              <h3 className="font-semibold text-gray-900 dark:text-white mb-4">项目成果</h3>
              <div className="space-y-2 text-sm">
                {project.metrics.users && (
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">用户数：</span>
                    <span className="text-gray-900 dark:text-white ml-2">{project.metrics.users.toLocaleString()}</span>
                  </div>
                )}
                {project.metrics.performance && (
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">性能：</span>
                    <span className="text-gray-900 dark:text-white ml-2">{project.metrics.performance}</span>
                  </div>
                )}
                {project.metrics.achievement && (
                  <div>
                    <span className="text-gray-600 dark:text-gray-400">成就：</span>
                    <span className="text-gray-900 dark:text-white ml-2">{project.metrics.achievement}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        {project.developmentProcess && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">开发过程</h2>
            <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-6">
              <div className="prose prose-blue dark:prose-invert max-w-none">
                <p className="text-gray-700 dark:text-gray-300">{project.developmentProcess}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {project.codeSnippets && project.codeSnippets.length > 0 && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">代码示例</h2>
                ...
                <div key={index} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900 dark:text-white mb-2">挑战 {index + 1}</h3>
                      <p className="text-gray-600 dark:text-gray-400 mb-3">{challenge}</p>
                      {project.solutions && project.solutions[index] && (
                        <div className="mt-4 bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <h4 className="font-medium text-gray-900 dark:text-white mb-1">解决方案</h4>
                              <p className="text-gray-600 dark:text-gray-400">{project.solutions[index]}</p>
                            </div>
                          </div>
                        </div>
                      )}
