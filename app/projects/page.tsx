...
import { ProjectCard }
from '@/components/features/ProjectCard' 
...
<option value="scale">按规模</option> </select> </div> </div> {/* 项目网格 */} {selectedCategory === 'all' ? ( // 显示所有项目，按分类分组 Object.entries(groupedProjects).map(([category, categoryProjects]) => ( <div key={category}
className="mb-16">
<div className="flex items-center gap-3 mb-8">
<div className="text-blue-600"> {categoryIcons[category as keyof typeof categoryIcons] || categoryIcons.other} </div>
<h2 className="text-2xl font-bold text-gray-900"> {categoryNames[category as keyof typeof categoryNames] || category} </h2> </div>
<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6"> {sortProjects(categoryProjects).map((project, index) => ( <LazyLoad key={project.id}
threshold={0.1}
rootMargin="200px" placeholder={ <div className="aspect-video bg-gray-100 rounded-lg animate-pulse" /> } >
<ProjectCard project={project}
/> </LazyLoad> ))} </div> </div> )) ) : ( // 显示筛选后的项目 <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6"> {sortedProjects.map((project, index) => ( <LazyLoad key={project.id}
threshold={0.1}
rootMargin="200px" placeholder={ <div className="aspect-video bg-gray-100 rounded-lg animate-pulse" /> } >
<ProjectCard project={project}
/> </LazyLoad> ))} </div> )} {sortedProjects.length === 0 && ( <div className="text-center py-12">
<p className="text-gray-500">该分类下暂无项目</p> </div> )} </PageContainer> ) }