...
jest.mock('@/components/dashboard/ContentDashboard', () => {
  return function ContentDashboard({ statistics }: any) {
    return (
      <div data-testid="content-dashboard">
        ...
        <div data-testid="statistics">
          ...
          <div>活跃用户: {statistics.users.activeCount}</div>
        </div>
      </div>
    )
  }
...
const mockStatistics = {
  ...
  tags: {
    popular: ['react', 'nextjs', 'typescript', 'design', 'productivity'],
    trending: ['ai', 'chatgpt', 'web3']
  },
  performance: {
    avgLoadTime: 1.2,
    avgTimeOnPage: 180,
...
describe('Dashboard Page', () => {
  ...
  it('renders ContentDashboard component', async () => {
  ...
  it('passes statistics to ContentDashboard', async () => {
    ;(getBlogStatistics as jest.Mock).mockResolvedValue(mockStatistics)
  ...
  it('calls getBlogStatistics on render', async () => {
    ;(getBlogStatistics as jest.Mock).mockResolvedValue(mockStatistics)
  ...
  it('renders with correct layout classes', async () => {
    ...
    expect(pageWrapper).toHaveClass('min-h-screen', 'bg-gray-50', 'dark:bg-gray-900', 'py-16')
  ...
  it('handles empty statistics gracefully', async () => {
    ...
    ;(getBlogStatistics as jest.Mock).mockResolvedValue(emptyStatistics)
  ...
  it('handles error from getBlogStatistics', async () => {
    const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation()
    ;(getBlogStatistics as jest.Mock).mockRejectedValue(new Error('Failed to fetch statistics'))

    await expect(DashboardPage()).rejects.toThrow('Failed to fetch statistics')
