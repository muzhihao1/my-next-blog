 ...
 * @property {string} slug - 标签slug
 * @property {number} postCount - 文章数量
 * @property {number} projectCount - 项目数量
 ...
 * @async
 * @function TagsPage
 * @returns {Promise<JSX.Element>} 渲染的标签索引页面
 * @description 显示所有标签的统计信息和标签云
 */
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {tags.map(tag => (
              <Link
                key={tag.slug}
                href={`/tags/${tag.slug}`}
                className="group block p-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-xl transition-all duration-300"
              >
                ...
                <div className="space-y-1 text-sm text-gray-600 dark:text-gray-400">
                  ...
                  {tag.postCount > 0 && (
                    <p>
                      <span className="font-medium text-gray-700 dark:text-gray-300">文章：</span>
                      {tag.postCount} 篇
                    </p>
                  )}
                  {tag.projectCount > 0 && (
                    <p>
                      <span className="font-medium text-gray-700 dark:text-gray-300">项目：</span>
                      {tag.projectCount} 个
                    </p>
                  )}
                </div>
