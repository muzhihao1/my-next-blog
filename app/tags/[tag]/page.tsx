...
function ContentCard({ item, type }: { item: BlogPost | Project; type: 'post' | 'project' }) {
  ...
  return (
    <Link href={href} className="block group">
      <article className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 h-full">
        {/* 缩略图 */}
        {((type === 'post' && (item as BlogPost).cover) || (type === 'project' && (item as Project).thumbnail)) && (
          <div className="relative aspect-video overflow-hidden">
            <Image
              src={(type === 'post' ? (item as BlogPost).cover : (item as Project).thumbnail) || ''}
              alt={item.title}
              fill
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            {type === 'post' && (item as BlogPost).category && (
            ...
            {type === 'project' && (
              <span className="text-sm text-gray-500 dark:text-gray-400">
                {(item as Project).status === 'active' ? '进行中' : 
                 (item as Project).status === 'completed' ? '已完成' : '已归档'}
              </span>
            )}
          </div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-4 line-clamp-2">
            {type === 'post' ? (item as BlogPost).excerpt : (item as Project).description}
          </p>
 ...
 * @param {Promise<{tag: string}>} props.params - 包含标签的参数
 ...
 * @param {Object} props - 组件属性
 * @param {Promise<{tag: string}>} props.params - 包含标签的参数
 * @returns {Promise<JSX.Element>} 渲染的标签详情页面
 * @description 显示所有带有特定标签的文章和项目
 */
        ...
        {taggedPosts.length > 0 && (
          <section className="mb-12">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">
              文章 ({taggedPosts.length})
            </h2>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {taggedPosts.map(post => (
                <ContentCard key={post.id} item={post} type="post" />
              ))}
            </div>
          </section>
        )}
