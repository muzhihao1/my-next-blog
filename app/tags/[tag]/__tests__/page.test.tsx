...
const mockPosts = [
  ...
  },
  {
    ...
    slug: 'nodejs-basics',
    excerpt: 'Getting started with Node.js',
...
describe('TagDetailPage', () => {
  ...
  it('renders tag detail page with posts and projects', async () => {
    ...
    render(await TagDetailPage({ params }))

    // Header
    expect(screen.getByText('#React')).toBeInTheDocument()
    expect(screen.getByText('共 3 个相关内容')).toBeInTheDocument()
  ...
  it('handles encoded tag names', async () => {
    const postsWithEncodedTags = [{
      ...mockPosts[0],
      tags: ['C++', 'Node.js']
    }]
    ;(getPosts as jest.Mock).mockResolvedValue(postsWithEncodedTags)
    ;(getProjects as jest.Mock).mockResolvedValue([])

    const params = Promise.resolve({ tag: 'nodejs' })
    render(await TagDetailPage({ params }))
  ...
  it('displays only posts when no projects have the tag', async () => {
    ;(getPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)

    const params = Promise.resolve({ tag: 'javascript' })
    render(await TagDetailPage({ params }))
  ...
  it('displays only projects when no posts have the tag', async () => {
    ;(getPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)

    const params = Promise.resolve({ tag: 'mongodb' })
    render(await TagDetailPage({ params }))
  ...
  it('calls notFound when tag has no content', async () => {
    ;(getPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)
  ...
  it('renders content cards with correct information', async () => {
    ;(getPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)
    ...
    // Post card details
    expect(screen.getByText('Learn Next.js basics')).toBeInTheDocument()
  ...
  it('renders correct links for content', async () => {
    ;(getPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)

    const params = Promise.resolve({ tag: 'react' })
    const { container } = render(await TagDetailPage({ params }))

    // Check post links
    const postLink = container.querySelector('a[href="/posts/nextjs-guide"]')
    expect(postLink).toBeInTheDocument()

    // Check project links
    const projectLink = container.querySelector('a[href="/projects/ecommerce-platform"]')
  ...
  it('renders content type badges correctly', async () => {
    ;(getPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)
  ...
  it('renders thumbnails when available', async () => {
    ;(getPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)
    ...
    // Check for images
    const postImage = screen.getByAltText('Next.js Guide')
  ...
  it('handles projects with different statuses', async () => {
    ;(getPosts as jest.Mock).mockResolvedValue([])
    ;(getProjects as jest.Mock).mockResolvedValue([
      { ...mockProjects[0], tags: ['Test'], status: 'completed' },
      { ...mockProjects[1], tags: ['Test'], status: 'active' },
      { ...mockProjects[1], id: 'p3', title: 'Archived', tags: ['Test'], status: 'archived' }
    ])

    const params = Promise.resolve({ tag: 'test' })
    render(await TagDetailPage({ params }))
...
describe('generateMetadata', () => {
  it('generates correct metadata for tag', async () => {
    const params = Promise.resolve({ tag: 'react' })
  ...
  it('handles encoded tags in metadata', async () => {
    const params = Promise.resolve({ tag: 'c%2B%2B' })
    const metadata = await generateMetadata({ params })
...
describe('generateStaticParams', () => {
  it('generates params for all unique tags', async () => {
    ;(getPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)

    const params = await generateStaticParams()

    // Should include all unique tags from posts and projects
    const tagSlugs = params.map(p => p.tag)
  ...
  it('uses fallback data when API fails', async () => {
    ...
    // Should return empty array when using empty fallback data
