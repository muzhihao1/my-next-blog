 ...
 * @module app/feed.json/route
 * @description 动态生成 JSON Feed 1.1 订阅源
 */
  ...
  try {
    ...
    const jsonFeed = {
      ...
      items: posts.slice(0, 20).map(post => ({
        ...
        date_modified: post.lastEditedTime ? new Date(post.lastEditedTime).toISOString() : new Date(post.date).toISOString(),
        authors: [{
