...
<div className="h-4 bg-gray-200 rounded w-64 mx-auto"></div> </div> </div> </div> ) }
return ( <div className="py-16 px-4 sm:px-6 lg:px-8">
<div className="max-w-7xl mx-auto"> {/* 页面标题 */}
<div className="text-center mb-12">
<h1 className="text-4xl font-bold text-gray-900 mb-4"> 我的收藏 </h1>
<p className="text-lg text-gray-600"> 共收藏了 {stats.total} 个内容 </p> </div> {/* 统计信息 */}
<div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
<div className="bg-white rounded-lg p-4 text-center">
<div className="text-2xl font-bold text-blue-600"> {stats.byType[FavoriteType.POST] || 0} </div>
<div className="text-sm text-gray-600">文章</div> </div>
<div className="bg-white rounded-lg p-4 text-center">
<div className="text-2xl font-bold text-green-600"> {stats.byType[FavoriteType.PROJECT] || 0} </div>
<div className="text-sm text-gray-600">项目</div> </div>
<div className="bg-white rounded-lg p-4 text-center">
<div className="text-2xl font-bold text-purple-600"> {stats.byType[FavoriteType.BOOK] || 0} </div>
<div className="text-sm text-gray-600">书籍</div> </div>
<div className="bg-white rounded-lg p-4 text-center">
<div className="text-2xl font-bold text-orange-600"> {stats.byType[FavoriteType.TOOL] || 0} </div>
<div className="text-sm text-gray-600">工具</div> </div> </div> {/* 筛选和搜索 */}
<div className="flex flex-col md:flex-row gap-4 mb-8"> {/* 类型筛选 */}
<div className="flex flex-wrap gap-2">
<button onClick={() => setSelectedType('all')}
className={`px-4 py-2 rounded-lg transition-colors ${ selectedType === 'all' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }`} > 全部 </button> {Object.values(FavoriteType).map(type => ( <button key={type}
onClick={() => setSelectedType(type)}
className={`px-4 py-2 rounded-lg transition-colors ${ selectedType === type ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }`} > {type === FavoriteType.POST ? '文章' : type === FavoriteType.PROJECT ? '项目' : type === FavoriteType.BOOK ? '书籍' : '工具'} </button> ))} </div> {/* 搜索框 */}
<div className="flex-1">
<input type="text" placeholder="搜索收藏..." value={searchQuery}
onChange={(e) => setSearchQuery(e.target.value)}
className="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" /> </div> {/* 排序选项 */}
<select value={sortBy}
onChange={(e) => setSortBy(e.target.value as 'date' | 'title')}
className="px-4 py-2 bg-white border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" >
<option value="date">按时间排序</option>
<option value="title">按标题排序</option> </select> {/* 清空按钮 */} {favorites.length > 0 && ( <button onClick={clearFavorites}
className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors" > 清空收藏 </button> )} </div> {/* 收藏列表 */} {filteredFavorites.length > 0 ? ( <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3"> {filteredFavorites.map(item => ( <FavoriteCard key={`${item.type}-${item.id}`}
item={item}
onRemove={() => { // 移除后会自动更新列表 }
}
...
<p className="text-gray-500 mb-4"> {searchQuery ? '没有找到匹配的收藏' : '还没有收藏任何内容'} </p> {!searchQuery && ( <Link href="/" className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors" > 去探索内容 </Link> )} </div> )} </div> </div> ) }