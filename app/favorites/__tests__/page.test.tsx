...
jest.mock('@/lib/hooks/useFavorites', () => ({
  ...
  formatFavoriteDate: jest.fn((date) => {
    const d = new Date(date)
    return `${d.getFullYear()}-${(d.getMonth() + 1).toString().padStart(2, '0')}-${d.getDate().toString().padStart(2, '0')}`
  }),
  getFavoriteStats: jest.fn((favorites) => ({
    total: favorites.length,
    byType: favorites.reduce((acc: any, item: any) => {
...
jest.mock('@/components/features/FavoriteButton', () => ({
  __esModule: true,
  default: ({ itemId, onToggle, size, showText }: any) => (
    <button 
      data-testid={`favorite-button-${itemId}`}
      onClick={onToggle}
      data-size={size}
      data-show-text={showText}
    >
      {showText !== false ? 'Remove' : '❤️'}
    </button>
  )
}))
...
const mockFavorites = [
  ...
  },
  {
    ...
    favoriteDate: new Date('2024-01-14')
  },
  {
    id: 'book-1',
    ...
    thumbnail: '/images/clean-code.jpg',
    favoriteDate: new Date('2024-01-13')
  },
  {
    id: 'tool-1',
    type: FavoriteType.TOOL,
    title: 'VS Code',
...
describe('Favorites Page', () => {
  const mockUseFavorites = {
    ...
    getFavoritesByType: jest.fn((type) => 
  ...
  it('renders page title and total count', () => {
  ...
  it('renders statistics for each type', () => {
    const { container } = render(<FavoritesPage />)
    ...
    // Check that all type stats are shown
    const statBoxes = statsGrid?.querySelectorAll('.bg-white.dark\\:bg-gray-800')
    expect(statBoxes).toHaveLength(4)
    
    // Check labels exist (there are multiple instances of each)
  ...
  it('filters favorites by type when type button is clicked', async () => {
    const user = userEvent.setup()
    render(<FavoritesPage />)
    
    // Click on 文章 (posts) filter
    const postFilterButton = screen.getAllByText('文章')[1] // Second one is the filter button
    await user.click(postFilterButton)
  ...
  it('shows all items when 全部 is clicked', async () => {
    const user = userEvent.setup()
    render(<FavoritesPage />)
    
    // First filter by type
    await user.click(screen.getAllByText('文章')[1])
    expect(screen.queryByText('E-commerce Platform')).not.toBeInTheDocument()
  ...
  it('searches favorites by title', async () => {
    ...
    // Should only show items matching search
    expect(screen.getByText('React Best Practices')).toBeInTheDocument()
    expect(screen.queryByText('E-commerce Platform')).not.toBeInTheDocument()
    expect(screen.queryByText('Clean Code')).not.toBeInTheDocument()
    expect(screen.queryByText('VS Code')).not.toBeInTheDocument()
  ...
  it('searches favorites by description', async () => {
    const user = userEvent.setup()
    render(<FavoritesPage />)
    
    const searchInput = screen.getByPlaceholderText('搜索收藏...')
    await user.type(searchInput, 'agile')
    
    // Should show items with matching description
    expect(screen.getByText('Clean Code')).toBeInTheDocument()
    expect(screen.queryByText('React Best Practices')).not.toBeInTheDocument()
  })

  it('sorts favorites by date (default)', () => {
    render(<FavoritesPage />)
  ...
  it('sorts favorites by title when selected', async () => {
    const user = userEvent.setup()
    render(<FavoritesPage />)
  ...
  it('calls clearFavorites when clear button is clicked', async () => {
    const user = userEvent.setup()
    render(<FavoritesPage />)
  ...
  it('renders empty search state when no matches', async () => {
    const user = userEvent.setup()
    render(<FavoritesPage />)
    ...
    expect(screen.getByText('没有找到匹配的收藏')).toBeInTheDocument()
    // Should not show explore button when searching
    expect(screen.queryByText('去探索内容')).not.toBeInTheDocument()
  })

  it('renders favorite cards with correct links', () => {
    render(<FavoritesPage />)
    
    // Check post link
    const postLink = screen.getAllByRole('link', { name: /React Best Practices/ })[0]
    expect(postLink).toHaveAttribute('href', '/posts/react-best-practices')
    
    // Check project link
    const projectLink = screen.getAllByRole('link', { name: /E-commerce Platform/ })[0]
    expect(projectLink).toHaveAttribute('href', '/projects/ecommerce-platform')
    
    // Check book link
    const bookLink = screen.getAllByRole('link', { name: /Clean Code/ })[0]
    expect(bookLink).toHaveAttribute('href', '/bookshelf/book-1')
    
    // Check tool link
    const toolLink = screen.getAllByRole('link', { name: /VS Code/ })[0]
  ...
  it('renders favorite dates correctly', () => {
    render(<FavoritesPage />)
  ...
  it('applies correct type colors to badges', () => {
    const { container } = render(<FavoritesPage />)
    
    // Find badges within the favorite cards (not the filter buttons)
    const favoriteCards = container.querySelectorAll('.group.relative.bg-white')
    
    // Check first card (post type)
    const postBadge = favoriteCards[0].querySelector('.bg-blue-100')
    expect(postBadge).toBeInTheDocument()
    expect(postBadge).toHaveTextContent('文章')
    
    // Check second card (project type)  
    const projectBadge = favoriteCards[1].querySelector('.bg-green-100')
    expect(projectBadge).toBeInTheDocument()
    expect(projectBadge).toHaveTextContent('项目')
    
    // Check third card (book type)
    const bookBadge = favoriteCards[2].querySelector('.bg-purple-100')
    expect(bookBadge).toBeInTheDocument()
    expect(bookBadge).toHaveTextContent('书籍')
    
    // Check fourth card (tool type)
  ...
  it('combines filtering and searching', async () => {
    const user = userEvent.setup()
    render(<FavoritesPage />)
    
    // First filter by project type
    await user.click(screen.getAllByText('项目')[1])
    
    // Then search
    const searchInput = screen.getByPlaceholderText('搜索收藏...')
    await user.type(searchInput, 'commerce')
