...
describe('/api/auth/user', () => {
  ...
  it('returns user data with profile', async () => {
    const mockUser = {
      id: 'user-123',
  ...
  it('uses user metadata avatar_url when profile avatar_url is not available', async () => {
    const mockUser = {
      id: 'user-123',
      email: 'test@example.com',
      created_at: '2024-01-01T00:00:00Z',
    ...
    // Should use user metadata avatar_url since profile doesn't have one
    expect(data.user.avatar_url).toBe('https://example.com/metadata-avatar.jpg')
  })
  ...
  it('handles profile query error gracefully', async () => {
    ...
    // Should still return user data even if profile query fails
