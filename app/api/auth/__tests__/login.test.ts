...
describe('/api/auth/login', () => {
  ...
  it('successfully initiates GitHub OAuth', async () => {
  ...
  it('rejects unsupported providers', async () => {
    ...
    const formData = new FormData()
    formData.append('provider', 'unsupported')
  ...
  it('handles empty OAuth response', async () => {
    const mockSignInWithOAuth = jest.fn().mockResolvedValue({
      data: {},
      error: null
    })
  ...
  it('uses correct redirect URL from request', async () => {
    const mockSignInWithOAuth = jest.fn().mockResolvedValue({
      data: { url: 'https://github.com/oauth/authorize?...' },
      error: null
    })
