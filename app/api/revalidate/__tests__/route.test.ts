...
describe('/api/revalidate', () => {
  ...
  const mockSecret = 'test-secret-token'

  beforeEach(() => {
    jest.clearAllMocks()
    process.env = { ...originalEnv, REVALIDATE_SECRET: mockSecret }
  ...
  describe('POST /api/revalidate', () => {
    it('revalidates path when type is "path"', async () => {
    ...
    it('revalidates tag when type is "tag"', async () => {
      const request = new NextRequest('http://localhost:3000/api/revalidate', {
        method: 'POST',
        headers: {
          'authorization': `Bearer ${mockSecret}`
    ...
    it('revalidates common paths when type is "all"', async () => {
      const request = new NextRequest('http://localhost:3000/api/revalidate', {
        method: 'POST',
        headers: {
          'authorization': `Bearer ${mockSecret}`
    ...
    it('returns 400 for missing path in path revalidation', async () => {
      const request = new NextRequest('http://localhost:3000/api/revalidate', {
        method: 'POST',
        headers: {
          'authorization': `Bearer ${mockSecret}`
    ...
    it('returns 400 for missing tag in tag revalidation', async () => {
      const request = new NextRequest('http://localhost:3000/api/revalidate', {
        method: 'POST',
        headers: {
          'authorization': `Bearer ${mockSecret}`
    ...
    it('uses default secret when REVALIDATE_SECRET is not set', async () => {
    ...
    it('logs revalidation requests', async () => {
      const request = new NextRequest('http://localhost:3000/api/revalidate', {
        method: 'POST',
        headers: {
          'authorization': `Bearer ${mockSecret}`
      ...
      expect(console.log).toHaveBeenCalledWith('Revalidation request:', {
        type: 'tag',
        path: undefined,
        tag: 'products'
      })
  ...
  describe('GET /api/revalidate', () => {
    it('returns 405 in production environment', async () => {
    ...
    it('revalidates all paths in development with valid secret', async () => {
    ...
    it('returns 401 for invalid secret in GET request', async () => {
      process.env.NODE_ENV = 'development'

      const request = new NextRequest('http://localhost:3000/api/revalidate?secret=wrong-secret')
      const response = await GET(request)
      const data = await response.json()
    ...
    it('returns 401 for missing secret in GET request', async () => {
      process.env.NODE_ENV = 'development'
