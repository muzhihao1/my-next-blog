...
describe('/api/comments', () => {
  ...
  describe('POST /api/comments', () => {
    ...
    it('returns error for content exceeding 500 characters', async () => {
      mockCreateClient.mockResolvedValue({
        auth: {
          getUser: jest.fn().mockResolvedValue({
            data: { user: mockUser },
    ...
    it('creates a reply comment with parent validation', async () => {
      const mockGetUser = jest.fn().mockResolvedValue({
        data: { user: mockUser },
        error: null
      })
      ...
      const mockSingle = jest.fn()
        .mockResolvedValueOnce({ data: { id: 'parent-comment' }, error: null }) // Parent exists
