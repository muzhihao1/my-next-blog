...
describe('/api/comments/[contentId]', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  ...
  describe('GET /api/comments/[contentId]', () => {
    const mockComments = [
      {
        id: 'comment-1',
        content: '这是第一条评论',
    ...
    it('uses default values for pagination', async () => {
      mockCreateClient.mockResolvedValue({
        from: jest.fn().mockReturnValue({
          select: jest.fn().mockReturnThis(),
          eq: jest.fn().mockReturnThis(),
    ...
    it('calculates hasMore correctly', async () => {
      mockCreateClient.mockResolvedValue({
        from: jest.fn().mockReturnValue({
          ...
          order: jest.fn().mockReturnThis(),
          range: jest.fn().mockResolvedValue({
            data: Array(10).fill(mockComments[0]),
            error: null,
            count: 50
          })
        })
    ...
    it('handles different content types', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/comments/project-1?contentType=project')
      await GET(request, { params: { contentId: 'project-1' } })

      // Verify contentType was used correctly
      expect(mockEq).toHaveBeenCalledWith('content_type', 'project')
    })
    ...
    it('constructs comment tree structure correctly', async () => {
      const commentsWithNullReplies = [
        {
          ...mockComments[0],
          replies: null // Test null handling
        }
      ]
