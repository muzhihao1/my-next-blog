...
const { Client: MockClient } = require('@notionhq/client')
...
describe('/api/health', () => {
  ...
  it('returns healthy status when all checks pass', async () => {
    ...
    process.env.NOTION_PROJECTS_DB = 'test-projects-db'
    process.env.NOTION_BOOKS_DB = 'test-books-db'
    process.env.NOTION_TOOLS_DB = 'test-tools-db'
    process.env.NODE_ENV = 'test'
    process.env.npm_package_version = '1.2.3'

    const mockMe = jest.fn().mockResolvedValue({ id: 'user-123' })
  ...
  it('returns unhealthy status when Notion token is missing', async () => {
  ...
  it('returns unhealthy status when Notion database ID is missing', async () => {
    process.env.NOTION_TOKEN = 'test-token'
  ...
  it('returns unhealthy status when Notion API fails', async () => {
    process.env.NOTION_TOKEN = 'test-token'
    process.env.NOTION_DATABASE_ID = 'test-database-id'

    const mockMe = jest.fn().mockRejectedValue(new Error('API request failed'))
    MockClient.mockImplementation(() => ({
      users: {
        me: mockMe
      }
  ...
  it('handles non-Error exceptions from Notion API', async () => {
    process.env.NOTION_TOKEN = 'test-token'
    process.env.NOTION_DATABASE_ID = 'test-database-id'

    const mockMe = jest.fn().mockRejectedValue('String error')
    MockClient.mockImplementation(() => ({
      users: {
        me: mockMe
      }
  ...
  it('correctly reports missing optional database IDs', async () => {
    process.env.NOTION_TOKEN = 'test-token'
    process.env.NOTION_DATABASE_ID = 'test-database-id'
    delete process.env.NOTION_PROJECTS_DB
    delete process.env.NOTION_BOOKS_DB
    delete process.env.NOTION_TOOLS_DB
    ...
    expect(data.status).toBe('healthy') // Still healthy as these are optional
    expect(data.checks.environment.hasProjectsDb).toBe(false)
