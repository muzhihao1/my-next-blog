...
export async function POST(request: NextRequest) {
  try {
    ...
    // 验证日期范围
    const daysDiff = (config.time_range.end.getTime() - config.time_range.start.getTime()) / (1000 * 60 * 60 * 24)
    if (daysDiff > 90) {
      return NextResponse.json(
    ...
    switch (config.format) {
      ...
      case 'csv':
        ...
        if (exportData.total_views !== undefined) {
          ...
          csvData.push({
            metric: '会话数',
            value: exportData.total_sessions,
          })
          csvData.push({
            metric: '平均会话时长',
        ...
        if (exportData.device_breakdown) {
          Object.entries(exportData.device_breakdown).forEach(([device, count]) => {
            csvData.push({
              metric: `设备 - ${device}`,
              value: count,
            })
      ...
      case 'excel':
        ...
        const overviewData = [
          ...
          ['平均会话时长', `${Math.round(exportData.avg_session_duration || 0)}秒`],
          ...
          ['参与率', `${((exportData.engagement_rate || 0) * 100).toFixed(2)}%`],
        ...
        if (exportData.device_breakdown) {
          const deviceData = [
            ['设备类型', '数量'],
            ...Object.entries(exportData.device_breakdown).map(([device, count]) => [device, count])
          ]
        ...
        if (exportData.browser_breakdown) {
          const browserData = [
            ['浏览器', '数量'],
            ...Object.entries(exportData.browser_breakdown).map(([browser, count]) => [browser, count])
          ]
        ...
        if (exportData.country_breakdown) {
          const countryData = [
            ['国家', '数量'],
            ...Object.entries(exportData.country_breakdown).map(([country, count]) => [country, count])
          ]
        ...
        if (exportData.raw_events && exportData.raw_events.length > 0) {
          const rawData = exportData.raw_events.map((event: any) => flattenObject(event))
