...
describe('/api/analytics/stats', () => {
  ...
  describe('GET /api/analytics/stats', () => {
    const mockActions = [
      {
        action_type: 'view',
        metadata: { scrollDepth: 50, readingTime: 120 },
        created_at: '2024-01-01T00:00:00Z'
      },
      {
        action_type: 'view',
        metadata: { scrollDepth: 100, readingTime: 180 },
    ...
    it('returns analytics stats for content', async () => {
      ...
      const mockFrom = jest.fn().mockImplementation((table) => {
        ...
        // For likes and bookmarks tables
        return {
          select: jest.fn().mockReturnThis(),
          eq: jest.fn().mockReturnThis(),
          count: table === 'likes' ? 15 : 8,
          error: null
        }
      })
      ...
      expect(data.stats).toEqual({
        ...
        bookmarks: 8,
        shares: 1,
        scrollDepth: { 25: 1, 50: 1, 75: 0, 100: 1 },
        averageReadingTime: 150 // (120 + 180) / 2
      })
      expect(response.status).toBe(200)
    })

    it('returns error for missing parameters', async () => {
      const request = new NextRequest('http://localhost:3000/api/analytics/stats?contentId=post-1')
      const response = await GET(request)
      const data = await response.json()
    ...
    it('handles different time periods correctly', async () => {
      ...
      // Test day period
      const dayRequest = new NextRequest('http://localhost:3000/api/analytics/stats?contentId=post-1&contentType=post&period=day')
      await GET(dayRequest)

      // Check that gte was called with a date 24 hours ago
    ...
    it('handles "all" period without date filter', async () => {
      ...
      createAdminClient.mockReturnValue({
        from: jest.fn().mockReturnValue({
          select: mockSelect,
          eq: mockEq,
          // Note: no gte method called for 'all' period
          ...mockQuery
        })
      })

      const request = new NextRequest('http://localhost:3000/api/analytics/stats?contentId=post-1&contentType=post&period=all')
      await GET(request)

      // Verify that gte was not called (no date filtering)
    ...
    it('handles database errors gracefully', async () => {
      ...
      // Create a mock that has the error property to match the actual query result structure
      const mockQuery = {
        select: mockSelect,
        eq: mockEq,
        data: null,
    ...
    it('calculates stats correctly with no actions', async () => {
      createAdminClient.mockReturnValue({
    ...
    it('handles actions without metadata gracefully', async () => {
      const actionsWithoutMetadata = [
        { action_type: 'view', metadata: null },
        { action_type: 'view', metadata: {} },
        ...
        from: jest.fn().mockImplementation((table) => {
          if (table === 'user_actions') {
            return {
              select: jest.fn().mockReturnThis(),
              eq: jest.fn().mockReturnThis(),
              data: actionsWithoutMetadata,
              count: actionsWithoutMetadata.length,
              error: null
            }
          }
          return {
    ...
    it('handles month period correctly', async () => {
      const mockSelect = jest.fn().mockReturnThis()
      const mockEq = jest.fn().mockReturnThis()
      const mockGte = jest.fn().mockResolvedValue({
        data: [],
      ...
      const request = new NextRequest('http://localhost:3000/api/analytics/stats?contentId=post-1&contentType=post&period=month')
      await GET(request)
