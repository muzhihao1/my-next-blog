...
function validateEvent(event: any): event is AnalyticsEvent {
  ...
  // 必需字段
  if (!event.event_type || !event.timestamp || !event.session_id || !event.anonymous_id) {
    return false
  }
    ...
    for (const sessionId of sessionIds) {
      ...
      await supabase
        ...
        .upsert({
          ...
          started_at: new Date(Math.min(...sessionEvents.map(e => new Date(e.timestamp).getTime()))),
          last_activity_at: new Date(),
          page_views: sessionEvents.filter(e => e.event_type === EventType.PAGE_VIEW).length,
