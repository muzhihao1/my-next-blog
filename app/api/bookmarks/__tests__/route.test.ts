...
describe('/api/bookmarks', () => {
  ...
  describe('GET /api/bookmarks', () => {
    it('returns bookmark count and status', async () => {
      const mockSelect = jest.fn().mockReturnThis()
      const mockEq = jest.fn().mockReturnThis()
      const mockSingle = jest.fn().mockResolvedValue({ data: { id: 'bookmark-1' }, error: null })

      mockCreateClient.mockResolvedValue({
        from: jest.fn().mockReturnValue({
          select: mockSelect.mockImplementation((_, options) => {
    ...
    it('returns error for missing parameters', async () => {
      const request = new NextRequest('http://localhost:3000/api/bookmarks?contentId=post-1')
      const response = await GET(request)
      const data = await response.json()
    ...
    it('returns isBookmarked false when user has not bookmarked', async () => {
      const mockSelect = jest.fn().mockReturnThis()
      const mockEq = jest.fn().mockReturnThis()
  ...
  describe('POST /api/bookmarks', () => {
    ...
    it('adds a bookmark successfully', async () => {
      ...
      const mockSingle = jest.fn()
        ...
        .mockResolvedValueOnce({ data: { id: 'bookmark-1' }, error: null }) // New bookmark created
    ...
    it('returns error if already bookmarked', async () => {
      mockCreateClient.mockResolvedValue({
        auth: {
          getUser: jest.fn().mockResolvedValue({
            data: { user: mockUser },
    ...
    it('returns error for missing parameters', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/bookmarks', {
        method: 'POST',
        body: JSON.stringify({ contentId: 'post-1' })
      })
  ...
  describe('DELETE /api/bookmarks', () => {
    ...
    it('returns error for missing parameters', async () => {
      ...
      const request = new NextRequest('http://localhost:3000/api/bookmarks?contentType=post', {
        method: 'DELETE'
      })
    ...
    it('handles database delete error', async () => {
      mockCreateClient.mockResolvedValue({
        auth: {
          getUser: jest.fn().mockResolvedValue({
            data: { user: mockUser },
