...
import { getSiteStatistics, getPostStatistics } from '@/lib/statistics'
...
describe('/api/statistics', () => {
  ...
  describe('GET', () => {
    it('should return site statistics by default', async () => {
      const mockStats = {
        totalPosts: 10,
        totalViews: 5000,
        totalTags: 15,
    ...
    it('should return post statistics when type=posts', async () => {
      const mockStats = {
        total: 20,
        categories: { tech: 10, life: 10 },
        tags: { javascript: 5, react: 5 },
        totalWords: 50000,
        averageWords: 2500,
        totalReadingTime: 250
      }

      ;(getPostStatistics as jest.Mock).mockResolvedValue(mockStats)
    ...
    it('should handle errors gracefully', async () => {
      ...
      ;(getSiteStatistics as jest.Mock).mockRejectedValue(new Error(errorMessage))
    ...
    it('should respect query parameters', async () => {
      ...
      const request = new NextRequest('http://localhost/api/statistics?limit=5&period=week')
      await GET(request)

      expect(getSiteStatistics).toHaveBeenCalledWith({
        limit: 5,
        period: 'week'
      })
    })
