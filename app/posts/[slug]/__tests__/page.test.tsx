...
import Post, { generateStaticParams, generateMetadata } from '../page'
import { getPostBySlug, getAllPostSlugs, withFallback, formatDate } from '@/lib/notion'
...
jest.mock('@/lib/fallback-posts', () => ({
  getFallbackPostBySlug: jest.fn(() => null)
}))
...
jest.mock('next/image', () => {
  return function Image({ src, alt, fill, ...props }: any) {
...
jest.mock('@/components/seo/SEO', () => ({
  SEO: ({ structuredData }: any) => <script type="application/ld+json">{JSON.stringify(structuredData)}</script>,
  generateArticleStructuredData: jest.fn((data) => ({ ...data, '@type': 'Article' }))
}))

jest.mock('@/components/features/TableOfContents', () => {
  return function TableOfContents({ content }: any) {
    return <div data-testid="table-of-contents">Table of Contents</div>
  }
})

jest.mock('@/components/features/ReadingProgress', () => {
  return function ReadingProgress({ showPercentage, offset }: any) {
    return <div data-testid="reading-progress" data-show-percentage={showPercentage} data-offset={offset} />
  }
})

jest.mock('@/components/features/SocialShare', () => ({
  SocialShare: ({ url, title }: any) => (
    <div data-testid="social-share" data-url={url} data-title={title}>Share</div>
  )
}))

jest.mock('@/components/features/ArticleReactions', () => {
  return function ArticleReactions({ contentId }: any) {
    return <div data-testid="article-reactions" data-content-id={contentId}>Reactions</div>
  }
})

jest.mock('@/components/features/TagList', () => {
  return function TagList({ tags, size }: any) {
    return (
      <div data-testid="tag-list" data-size={size}>
...
jest.mock('@/components/features/FavoriteButton', () => ({
  __esModule: true,
  default: ({ itemId, title, size }: any) => (
    <button data-testid="favorite-button" data-item-id={itemId} data-size={size}>{title}</button>
  ),
  FloatingFavoriteButton: ({ itemId }: any) => (
    <button data-testid="floating-favorite-button" data-item-id={itemId}>Floating Favorite</button>
  )
}))

jest.mock('@/components/ui/Container', () => ({
  ContentContainer: ({ children }: any) => <div className="content-container">{children}</div>
}))

jest.mock('@/components/comments/CommentSection', () => ({
  CommentSection: ({ contentId }: any) => (
    <div data-testid="comment-section" data-content-id={contentId}>Comments</div>
  )
}))

jest.mock('@/lib/utils/content', () => ({
  calculateWordCount: jest.fn((content) => content.length),
  formatUpdateTime: jest.fn((date, lastEdited) => lastEdited ? 'Updated' : null)
}))
...
describe('Post Page', () => {
  ...
  describe('generateStaticParams', () => {
    it('returns slugs from Notion when available', async () => {
      process.env.NOTION_TOKEN = 'test-token'
      process.env.NOTION_DATABASE_ID = 'test-db'
      ;(getAllPostSlugs as jest.Mock).mockResolvedValue(['slug1', 'slug2', 'slug3'])
    ...
    it('returns fallback slugs when Notion fails', async () => {
      process.env.NOTION_TOKEN = 'test-token'
      process.env.NOTION_DATABASE_ID = 'test-db'
      ;(getAllPostSlugs as jest.Mock).mockRejectedValue(new Error('API Error'))
    ...
    it('returns fallback slugs when environment not configured', async () => {
  ...
  describe('generateMetadata', () => {
    it('generates metadata for existing post', async () => {
      ...
      expect(metadata.openGraph).toMatchObject({
        title: 'Test Post Title',
        description: 'This is the post excerpt',
        type: 'article',
    ...
    it('generates 404 metadata for non-existent post', async () => {
  ...
  describe('Post Component', () => {
    it('renders post content correctly', async () => {
      ...
      render(await Post({ params: Promise.resolve({ slug: 'test-post' }) }))
    ...
    it('calls notFound when post does not exist', async () => {
      ...
      await Post({ params: Promise.resolve({ slug: 'non-existent' }) })
    ...
    it('does not render cover image when not available', async () => {
      const postWithoutCover = { ...mockPost, cover: null }
      ;(getPostBySlug as jest.Mock).mockResolvedValue(postWithoutCover)
    ...
    it('renders article reactions', async () => {
      ;(getPostBySlug as jest.Mock).mockResolvedValue(mockPost)
    ...
    it('renders favorite buttons', async () => {
      ;(getPostBySlug as jest.Mock).mockResolvedValue(mockPost)
    ...
    it('applies correct category class', async () => {
      const categories = ['Technology', 'Design', 'Productivity', 'Life', 'Unknown']
      
      for (const category of categories) {
        const postWithCategory = { ...mockPost, category }
        ;(getPostBySlug as jest.Mock).mockResolvedValue(postWithCategory)

        const { rerender } = render(await Post({ params: Promise.resolve({ slug: 'test-post' }) }))

        const categoryBadge = screen.getByText(category)
        if (category === 'Unknown') {
          expect(categoryBadge).toHaveClass('category-badge', 'category-technology')
        } else {
          expect(categoryBadge).toHaveClass('category-badge', `category-${category.toLowerCase()}`)
        }
    ...
    it('does not render update time when not available', async () => {
      const postWithoutUpdate = { ...mockPost, lastEditedTime: null }
      ;(getPostBySlug as jest.Mock).mockResolvedValue(postWithoutUpdate)
