...
offset={80}
/>
<ContentContainer> {/* 文章头部 */}
<header className="mb-16 mt-8"> {/* 分类标签 */}
<div className="mb-8 text-center">
<span className={getCategoryClass(post.category)}> {post.category} </span> </div> {/* 文章标题 */}
<h1 className="text-4xl md:text-6xl font-light leading-tight mb-8 text-center text-gray-900 tracking-tight"> {post.title} </h1> {/* 文章摘要 */} {post.excerpt && ( <p className="text-xl text-gray-600 leading-relaxed text-center mb-12 max-w-3xl mx-auto"> {post.excerpt} </p> )} {/* 作者信息和元数据 */}
<div className="flex flex-col sm:flex-row items-center justify-center gap-6 mb-8">
<div className="flex items-center">
<div className="relative w-12 h-12 mr-4">
<Image src={post.author.avatar}
alt={post.author.name}
fill className="rounded-full object-cover" /> </div>
...
alt={post.title}
fill className="object-cover rounded-2xl shadow-2xl" /> </div> )} {/* 分割线 */}
<div className="divider"></div> </header> {/* 文章内容区域 */}
<div className="grid grid-cols-1 lg:grid-cols-[1fr_300px]
gap-8 max-w-6xl mx-auto"> {/* 文章内容 */}
<div className="max-w-3xl">
<div className="prose-blog" dangerouslySetInnerHTML={{ __html: post.content || '<p>内容加载中...</p>' }
}
/> </div> {/* 侧边栏目录 */}
...
d="M10 19l-7-7m0 0l7-7m-7 7h18" /> </svg> 返回首页 </Link> </div> {/* 评论区域 */}
...
<input type="email" placeholder="输入您的邮箱地址" className="flex-1 px-4 py-3 border border-gray-300 bg-white text-gray-900 rounded-lg focus:outline-none focus:border-pink-500:border-pink-400 focus:ring-2 focus:ring-pink-200:ring-pink-800 transition-all" />
<button type="submit" className="btn-subscribe" > 订阅 </button> </form>
<p className="text-xs text-gray-500 mt-4"> 每周最多一封邮件，随时可以取消订阅 </p> </div> </footer> </ContentContainer> {/* 浮动收藏按钮 */}
