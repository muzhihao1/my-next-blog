...
jest.mock('@/lib/notion', () => ({
  ...
  withFallback: jest.fn((fn, fallback) => fn()),
...
jest.mock('@/components/widgets/StatsWidget', () => {
  return function StatsWidget() {
    return <div data-testid="stats-widget">Stats Widget</div>
  }
})

jest.mock('@/components/features/HomeClient', () => {
  return function HomeClient({ children }: any) {
...
describe('Home Page', () => {
  ...
  it('renders hero section with correct content', async () => {
    ...
    expect(screen.getByText(/在这里，我分享技术见解/)).toBeInTheDocument()
  ...
  it('renders client components', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)
  ...
  it('renders featured projects section with correct data', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)
    ...
    expect(screen.getByText('探索我最引以为豪的作品')).toBeInTheDocument()
  ...
  it('renders tech stack for projects', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue(mockProjects)
  ...
  it('renders recent posts section with correct data', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
    ...
    expect(screen.getByText('技术思考与经验分享')).toBeInTheDocument()

    // Should show first 3 posts
  ...
  it('renders post categories with correct styling', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue([])
  ...
  it('handles empty posts and projects gracefully', async () => {
  ...
  it('limits featured projects to 3', async () => {
    const manyProjects = Array(10).fill(null).map((_, i) => ({
      id: `${i}`,
      title: `Project ${i}`,
      slug: `project-${i}`,
      description: `Description ${i}`,
      thumbnail: `/images/project${i}.jpg`,
      techStack: ['Tech'],
      featured: true
    }))

    ;(getPublishedPosts as jest.Mock).mockResolvedValue([])
    ;(getProjects as jest.Mock).mockResolvedValue(manyProjects)

    render(await Home())

    // Should only show first 3 featured projects
  ...
  it('limits recent posts to 3', async () => {
    ...
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(manyPosts)
    ;(getProjects as jest.Mock).mockResolvedValue([])
  ...
  it('renders post dates correctly', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
    ;(getProjects as jest.Mock).mockResolvedValue([])

    render(await Home())

    // formatDate should be called for each post
  ...
  it('applies correct styling classes', async () => {
    ...
    // Check for section backgrounds
    expect(container.querySelector('.bg-gray-50.dark\\:bg-gray-900')).toBeInTheDocument()
  })
})