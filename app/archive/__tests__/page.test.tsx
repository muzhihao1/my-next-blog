...
const mockPosts = [
  ...
  },
  {
    id: '3',
    title: '2023年技术总结',
    slug: '2023-tech-summary',
    excerpt: '回顾2023年的技术成长',
    date: '2023-12-25',
    category: '总结',
    tags: ['年度总结']
  },
  ...
  fallbackPosts: [
    {
      ...
      title: 'Fallback Post',
      slug: 'fallback-post',
      excerpt: 'This is a fallback post',
      date: '2024-01-01',
      category: 'Test',
      tags: ['fallback']
    }
...
describe('ArchivePage', () => {
  ...
  it('renders archive page with posts grouped by year', async () => {
    ...
    expect(screen.getByText('所有文章按时间排序')).toBeInTheDocument()
    ...
    // Post counts - use getAllByText since there are multiple (2 篇)
    const twoPostsElements = screen.getAllByText('(2 篇)')
    expect(twoPostsElements).toHaveLength(2) // 2024 and 2023 both have 2 posts
    expect(screen.getByText('(1 篇)')).toBeInTheDocument() // 2022 has 1 post
  })

  it('displays posts under correct year groups', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
    ...
    // Check 2024 posts
    expect(year2024Section).toHaveTextContent('Next.js 15 新特性详解')
    expect(year2024Section).toHaveTextContent('React Server Components 实践')

    // Check 2023 posts
    expect(year2023Section).toHaveTextContent('2023年技术总结')
  ...
  it('formats dates correctly in Chinese format', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)

    await renderAsync(ArchivePage())

    // Check date formatting (toLocaleDateString in Chinese outputs MM/DD format)
    expect(screen.getByText('01/15')).toBeInTheDocument()
    expect(screen.getByText('01/10')).toBeInTheDocument()
    expect(screen.getByText('12/25')).toBeInTheDocument()
  })

  it('creates correct links for posts', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
  ...
  it('displays post excerpts', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
  ...
  it('applies hover styles classes', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
  ...
  it('uses fallback posts when API fails', async () => {
    ;(getPublishedPosts as jest.Mock).mockRejectedValue(new Error('API Error'))
  ...
  it('groups posts correctly when multiple posts in same year', async () => {
    const postsWithSameYear = [
      ...mockPosts,
      {
        id: '6',
        title: 'Another 2024 Post',
        slug: 'another-2024-post',
        excerpt: 'Another post from 2024',
        date: '2024-05-20',
        category: '技术',
        tags: ['Test']
      }
    ]
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(postsWithSameYear)

    await renderAsync(ArchivePage())

    // Should show 3 posts for 2024
    expect(screen.getByText('(3 篇)')).toBeInTheDocument()
  })

  it('handles single post in a year', async () => {
    const singlePost = [{
      id: '1',
      title: 'Single Post',
      slug: 'single-post',
      excerpt: 'Only post in 2024',
      date: '2024-06-15',
      category: '技术',
      tags: ['Single']
    }]
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(singlePost)
  ...
  it('preserves post order within year groups', async () => {
    const postsInOrder = [
      {
        id: '1',
        title: 'December Post',
        slug: 'december-post',
        excerpt: 'Post from December',
        date: '2024-12-25',
        category: '技术',
        tags: ['Test']
      },
    ...
    const year2024Section = screen.getByText('2024').closest('div')
    const articles = year2024Section?.querySelectorAll('article')
    
    // Posts should appear in the order they were returned (not re-sorted by date)
    expect(articles?.[0]).toHaveTextContent('December Post')
    expect(articles?.[1]).toHaveTextContent('January Post')
  })

  it('applies responsive spacing and layout', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)

    const { container } = await renderAsync(ArchivePage())

    const mainContainer = container.firstChild
    expect(mainContainer).toHaveClass('max-w-4xl', 'mx-auto', 'px-4', 'sm:px-6', 'lg:px-8')
  })

  it('handles posts from future years', async () => {
    const futurePost = [{
      id: '1',
      title: 'Future Post',
      slug: 'future-post',
      excerpt: 'Post from the future',
      date: '2025-01-01',
      category: '未来',
      tags: ['Future']
    }]
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(futurePost)
