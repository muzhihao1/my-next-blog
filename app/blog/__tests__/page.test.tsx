...
jest.mock('@/components/ui/Container', () => ({
  PageContainer: ({ children, className, size }: any) => (
    <div className={className} data-size={size}>{children}</div>
  )
}))
...
const mockPosts = [
  ...
  },
  {
    ...
    excerpt: 'Learn about design principles',
    date: '2024-01-02',
    category: 'Design',
    tags: ['ui', 'ux'],
    author: 'Jane Smith',
  ...
  },
  {
    ...
    excerpt: 'Boost your productivity with these tips',
    date: '2024-01-03',
  ...
  },
  {
    ...
    title: 'Life Lessons',
    slug: 'life-lessons',
    excerpt: 'Important life lessons I learned',
    date: '2024-01-04',
    category: 'Life',
    tags: ['personal'],
    author: 'Alice Brown',
  ...
  },
  {
    ...
    excerpt: 'Post with unknown category',
    date: '2024-01-05',
    category: 'Random',
    tags: [],
    author: 'Test User',
...
describe('Blog Page', () => {
  ...
  it('renders category badges with correct classes', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
    ...
    // Unknown category should fall back to technology style
    const unknownCategory = screen.getByText('Random')
  ...
  it('renders post metadata correctly', async () => {
    ...
    // Check that formatDate is called for each post
    expect(formatDate).toHaveBeenCalledTimes(mockPosts.length)
    mockPosts.forEach(post => {
      expect(formatDate).toHaveBeenCalledWith(post.date)
    })
  ...
  it('renders tags for posts that have them', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)

    render(await BlogPage())

    // First post should show first 2 tags
    expect(screen.getByText('javascript')).toBeInTheDocument()
    expect(screen.getByText('react')).toBeInTheDocument()
    // Third tag should not be shown (limited to 2)
  ...
  it('does not render tag section for posts without tags', async () => {
  ...
  it('uses withFallback for data fetching', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
  ...
  it('renders posts in a responsive grid layout', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
  ...
  it('uses PageContainer with correct size', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
  ...
  it('limits tags display to 2 per post', async () => {
    ...
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(postWithManyTags)
  ...
  it('applies article-card class to posts', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue(mockPosts)
  ...
  it('renders metadata separator correctly', async () => {
    ;(getPublishedPosts as jest.Mock).mockResolvedValue([mockPosts[0]])

    render(await BlogPage())

    // Should have separator between date and read time
