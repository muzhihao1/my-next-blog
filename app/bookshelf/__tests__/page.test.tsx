...
// Mock dependencies
jest.mock('@/lib/notion/books', () => ({
  getBooks: jest.fn()
}))

jest.mock('../BookshelfClient', () => {
  return function MockBookshelfClient({ books }: { books: Book[] }) {
    return (
      <div data-testid="bookshelf-client">
        <h1>我的书架</h1>
        <div data-testid="books-count">{books.length} books</div>
...
describe('Bookshelf Page', () => {
  ...
  it('handles empty books array', async () => {
  ...
  it('calls getBooks exactly once', async () => {
    ;(getBooks as jest.Mock).mockResolvedValue(mockBooks)
...
// Mock additional dependencies for BookshelfClient
jest.mock('@/components/features/BookCard', () => {
  return function BookCard({ book, view }: any) {
    return (
      <div data-testid={`book-card-${book.id}`} data-view={view}>
        <h3>{book.title}</h3>
        <p>{book.author}</p>
...
jest.mock('@/components/features/ViewToggle', () => {
  return function ViewToggle({ view, onViewChange }: any) {
    return (
      <div>
        <button onClick={() => onViewChange('grid')} aria-label="grid view">Grid</button>
        <button onClick={() => onViewChange('list')} aria-label="list view">List</button>
      </div>
    )
  }
})
...
jest.mock('@/components/ui/LazyLoad', () => {
  return function LazyLoad({ children }: any) {
    return <div data-testid="lazy-load-wrapper">{children}</div>
  }
})

jest.mock('@/components/ui/Container', () => ({
  PageContainer: ({ children, size }: any) => (
    <div data-size={size} className="page-container">{children}</div>
  )
}))

describe('BookshelfClient Component', () => {
  // Import the real component for client-side testing
  const RealBookshelfClient = jest.requireActual('../BookshelfClient').default
  ...
  it('renders page title and description', () => {
    render(<RealBookshelfClient books={mockBooks} />)
  ...
  it('displays correct statistics', () => {
    ...
    const stats = container.querySelectorAll('.text-2xl.font-bold')
    expect(stats).toHaveLength(5)
    
    // Check specific labels exist (use getAllByText since they appear in multiple places)
    expect(screen.getAllByText('已读').length).toBeGreaterThan(0)
    expect(screen.getAllByText('在读').length).toBeGreaterThan(0)
    expect(screen.getAllByText('想读').length).toBeGreaterThan(0)

    // Average rating (only 'read' books with rating)
  ...
  it('filters books by status', async () => {
    ...
    // Filter by "已读" (read) - first select is status filter
    const selects = screen.getAllByRole('combobox')
    const statusSelect = selects[0]
    await user.selectOptions(statusSelect, 'read')

    // Should only show read books
    expect(screen.getByText('深入浅出设计模式')).toBeInTheDocument()
  ...
  it('filters books by category', async () => {
    ...
    await user.selectOptions(categorySelect, '技术')

    // Should only show books in 技术 category
    expect(screen.getByText('深入浅出设计模式')).toBeInTheDocument()
    expect(screen.getByText('代码整洁之道')).toBeInTheDocument()
    expect(screen.queryByText('人月神话')).not.toBeInTheDocument()
  })

  it('sorts books by date (default)', () => {
    render(<RealBookshelfClient books={mockBooks} />)

    const bookCards = screen.getAllByTestId(/^book-card-/)
    
    // Should be sorted by date (newest first)
    // Book 2 has the most recent start date (2024-02-01)
  ...
  it('sorts books by rating', async () => {
    ...
    expect(bookCards[0]).toHaveTextContent(/深入浅出设计模式|人月神话/)
  })
  ...
  it('sorts books by title', async () => {
    ...
    expect(bookCards[0]).toHaveTextContent('人月神话')
  ...
  it('shows empty state when no books match filters', async () => {
    ...
    // Filter by a category that has no books
    const categorySelect = screen.getAllByRole('combobox')[1]
    await user.selectOptions(categorySelect, '管理')
    
    // Then filter by a status that doesn't match
  ...
  it('calculates average rating correctly', () => {
    const booksWithRatings: Book[] = [
      { ...mockBooks[0], status: 'read', rating: 5 },
      { ...mockBooks[1], status: 'read', rating: 3 },
      { ...mockBooks[2], status: 'read', rating: 4 }
    ]
  ...
  it('handles books without optional fields', () => {
  ...
  it('renders LazyLoad wrappers for performance', () => {
    const { container } = render(<RealBookshelfClient books={mockBooks} />)

    // LazyLoad components should be present
    const lazyLoadElements = container.querySelectorAll('[data-testid="lazy-load-wrapper"]')
    expect(lazyLoadElements.length).toBe(mockBooks.length)
  })

  it('preserves filter state when sorting changes', async () => {
    const user = userEvent.setup()
    render(<RealBookshelfClient books={mockBooks} />)
