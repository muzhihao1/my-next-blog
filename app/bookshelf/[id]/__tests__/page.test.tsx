...
import BookDetailPage, { generateMetadata, generateStaticParams } from '../page'
...
jest.mock('@/components/seo/StructuredData', () => ({
  ...
  generateBookStructuredData: (data: any) => ({
    ...data,
    '@type': 'Book'
  })
}))

jest.mock('@/lib/fallback-books', () => ({
  fallbackBooks: [
    {
      id: '1',
      ...
      notes: '这是一本关于设计模式的书籍笔记。\n\n## 主要收获\n\n1. 单一职责原则',
      takeaways: '设计模式是解决常见问题的最佳实践',
      tags: ['设计模式', '软件工程'],
      publishYear: 2019,
      pages: 395,
      language: '中文'
...
describe('BookDetail Page', () => {
  ...
  it('renders book detail with all information', async () => {
    const params = Promise.resolve({ id: '1' })
    render(await BookDetailPage({ params }))

    // Title and author
    expect(screen.getByText('设计模式：可复用面向对象软件的基础')).toBeInTheDocument()
    expect(screen.getByText('Erich Gamma等')).toBeInTheDocument()

    // Category and publish info
    ...
    // Reading dates (formatted with toLocaleDateString)
    expect(screen.getByText(/2024/)).toBeInTheDocument()
    expect(screen.getByText('阅读时间')).toBeInTheDocument()

    // Pages
    expect(screen.getByText('395 页')).toBeInTheDocument()

    // ISBN
    expect(screen.getByText('9787111618973')).toBeInTheDocument()
    ...
    expect(screen.getByText('设计模式是解决常见问题的最佳实践')).toBeInTheDocument()
  ...
  it('renders rating stars correctly', async () => {
    ...
    expect(filledStars).toHaveLength(5) // 5 filled stars for rating 5
  })
  ...
  it('renders back to bookshelf link', async () => {
    const params = Promise.resolve({ id: '1' })
    render(await BookDetailPage({ params }))
  ...
  it('converts markdown notes to HTML', async () => {
    const params = Promise.resolve({ id: '1' })
    const { container } = render(await BookDetailPage({ params }))
  ...
  it('handles book with minimal information', async () => {
    ...
    expect(screen.queryByText('阅读时间')).not.toBeInTheDocument()
  ...
  it('calls notFound when book does not exist', async () => {
    ...
    await expect(BookDetailPage({ params })).rejects.toThrow('NEXT_NOT_FOUND')
    expect(notFound).toHaveBeenCalledTimes(1)
  })

  it('shows correct status color for different statuses', async () => {
    // Test "reading" status
    ...
    expect(readingStatus).toHaveTextContent('在读')

    // Clear previous render
    jest.clearAllMocks()

    // Test "want-to-read" status  
    const params3 = Promise.resolve({ id: '3' })
  ...
  it('handles empty notes gracefully', async () => {
    ...
    // Should not render notes section if no notes
    expect(screen.queryByText('读书笔记')).not.toBeInTheDocument()
  })
  ...
  it('handles markdown conversion errors gracefully', async () => {
    // Mock remark to throw error
    const mockRemark = jest.requireMock('remark')
    mockRemark.remark.mockReturnValueOnce({
      use: jest.fn().mockReturnValue({
        process: jest.fn().mockRejectedValue(new Error('Markdown error'))
      })
    })
  ...
  it('displays publish year when available', async () => {
    const params = Promise.resolve({ id: '1' })
    render(await BookDetailPage({ params }))
  ...
  it('displays language when available', async () => {
    const params = Promise.resolve({ id: '1' })
    render(await BookDetailPage({ params }))
...
describe('generateMetadata', () => {
  it('generates correct metadata for existing book', async () => {
    const params = Promise.resolve({ id: '1' })
  ...
  it('generates fallback metadata for non-existent book', async () => {
    const params = Promise.resolve({ id: 'non-existent' })
    const metadata = await generateMetadata({ params })
...
describe('generateStaticParams', () => {
  it('generates params for all fallback books', async () => {
