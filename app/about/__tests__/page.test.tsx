...
// Mock components
jest.mock('@/components/features/SkillsSection', () => ({
  __esModule: true,
  default: () => <div data-testid="skills-section">Skills Section Component</div>
}))

jest.mock('@/components/features/Timeline', () => ({
  __esModule: true,
  default: () => <div data-testid="timeline">Timeline Component</div>
}))

jest.mock('@/components/features/ContactInfo', () => ({
  __esModule: true,
  default: () => <div data-testid="contact-info">Contact Info Component</div>
}))

jest.mock('@/components/seo/SEO', () => ({
  SEO: ({ structuredData }: any) => (
    <div data-testid="seo" data-structured-data={JSON.stringify(structuredData)} />
  ),
  generatePersonStructuredData: (data: any) => ({
    '@type': 'Person',
    ...data
  })
}))
...
describe('About Page', () => {
  it('renders all sections correctly', () => {
    ...
    // Hero section
    expect(screen.getByText('你好，我是 Zhihao Mu')).toBeInTheDocument()
    expect(screen.getByText('全栈开发者 / 技术作家 / 终身学习者')).toBeInTheDocument()
    expect(screen.getByText('热爱技术，享受创造。专注于构建优雅的解决方案，分享知识与经验。')).toBeInTheDocument()

    // Story section
    expect(screen.getByText('我的故事')).toBeInTheDocument()
  ...
  it('renders hero section with avatar emoji', () => {
    render(<About />)
  ...
  it('renders all value cards', () => {
    ...
    // Value cards
    const values = [
      { emoji: '🎯', title: '专注品质', desc: '追求卓越，关注细节。相信好的产品来自于对品质的执着追求。' },
      { emoji: '🌱', title: '持续成长', desc: '保持好奇心，拥抱变化。技术日新月异，学习永无止境。' },
      { emoji: '🤝', title: '开放协作', desc: '相信团队的力量，乐于分享知识。开源精神，共同进步。' },
      { emoji: '💡', title: '创新思维', desc: '不满足于现状，勇于尝试新事物。用技术解决实际问题。' }
    ]

    values.forEach(value => {
      expect(screen.getByText(value.emoji)).toBeInTheDocument()
  ...
  it('renders SEO structured data', () => {
    render(<About />)
  ...
  it('applies correct styling to sections', () => {
    ...
    // Most sections have mb-16 spacing
  ...
  it('renders newsletter section with correct background', () => {
    const { container } = render(<About />)
  ...
  it('uses correct heading hierarchy', () => {
    ...
    expect(h1).toHaveTextContent('你好，我是 Zhihao Mu')

    // h2 for section titles
    const h2s = screen.getAllByRole('heading', { level: 2 })
    expect(h2s).toHaveLength(5) // 我的故事, 技术栈, 职业经历, 我的价值观, 联系我
  ...
  it('applies prose styling to story section', () => {
    render(<About />)

    const storySection = screen.getByText('我的故事').nextElementSibling
    expect(storySection).toHaveClass('prose', 'prose-gray', 'dark:prose-invert', 'max-w-none')
  })

  it('renders text with correct formatting', () => {
    render(<About />)

    // Check text sizes
    expect(screen.getByText('你好，我是 Zhihao Mu')).toHaveClass('text-4xl')
